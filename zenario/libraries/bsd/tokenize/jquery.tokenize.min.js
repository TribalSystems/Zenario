...
function(a){c.keyup(a)});this.searchInput.on("keypress",function(a){c.keypress(a)});this.searchInput.on("paste",function(){setTimeout(function(){c.resizeSearchInput()},10);setTimeout(function(){var a=c.searchInput.val().split(",");1<a.length&&b.each(a,function(a,b){c.tokenAdd(b.trim(),"")})},20)});b(document).on("click",function(){c.dropdownHide();1==c.options.maxElements&&c.searchInput.val()&&c.tokenAdd(c.searchInput.val(),"")});this.resizeSearchInput();this.remap(!0);this.updatePlaceholder()},updateOrder:function(){if(this.options.sortable){var a,
c,d=this;b.each(this.tokensContainer.sortable("toArray",{attribute:"data-value"}),function(e,g){c=b('option[value="'+g+'"]',d.select);void 0==a?c.prependTo(d.select):a.after(c);a=c});this.options.onReorder(this)}},updatePlaceholder:function(){this.options.placeholder&&(void 0==this.placeholder&&(this.placeholder=b("<li />").addClass("Placeholder").html(this.options.placeholder),this.placeholder.insertBefore(b("li:first-child",this.tokensContainer))),0==this.searchInput.val().length&&0==b("li.Token",
this.tokensContainer).length?this.placeholder.show():this.placeholder.hide())},dropdownShow:function(){this.dropdown.show()},dropdownPrev:function(){0<b("li.Hover",this.dropdown).length?b("li.Hover",this.dropdown).is("li:first-child")?(b("li.Hover",this.dropdown).removeClass("Hover"),b("li:last-child",this.dropdown).addClass("Hover")):b("li.Hover",this.dropdown).removeClass("Hover").prev().addClass("Hover"):b("li:first",this.dropdown).addClass("Hover")},dropdownNext:function(){0<b("li.Hover",this.dropdown).length?
b("li.Hover",this.dropdown).is("li:last-child")?(b("li.Hover",this.dropdown).removeClass("Hover"),b("li:first-child",this.dropdown).addClass("Hover")):b("li.Hover",this.dropdown).removeClass("Hover").next().addClass("Hover"):b("li:first",this.dropdown).addClass("Hover")},dropdownAddItem:function(a,c,d){d=d||c;if(!b('li[data-value="'+a+'"]',this.tokensContainer).length){var e=this,g=b("<li />").attr("data-value",a).attr("data-text",c).html(d).on("click",function(a){a.stopImmediatePropagation();e.tokenAdd(b(this).attr("data-value"),
...
this.resizeSearchInput()},resetPendingTokens:function(){b("li.PendingDelete",this.tokensContainer).removeClass("PendingDelete")},keypress:function(a){String.fromCharCode(a.which)==this.options.delimiter&&(a.preventDefault(),this.tokenAdd(this.searchInput.val(),""))},keydown:function(a){switch(a.keyCode){case 8:0==this.searchInput.val().length&&(a.preventDefault(),b("li.Token.PendingDelete",this.tokensContainer).length?this.tokenRemove(b("li.Token.PendingDelete").attr("data-value")):b("li.Token:last",
this.tokensContainer).addClass("PendingDelete"),this.dropdownHide());break;case 9:case 13:if(b("li.Hover",this.dropdown).length){var c=b("li.Hover",this.dropdown);a.preventDefault();this.tokenAdd(c.attr("data-value"),c.attr("data-text"))}else this.searchInput.val()&&(a.preventDefault(),this.tokenAdd(this.searchInput.val(),""));this.resetPendingTokens();break;case 27:this.resetSearchInput();this.dropdownHide();this.resetPendingTokens();break;case 38:a.preventDefault();this.dropdownPrev();break;case 40:a.preventDefault();
this.dropdownNext();break;default:0<this.options.maxElements&&b("li.Token",this.tokensContainer).length>=this.options.maxElements&&a.preventDefault(),this.resetPendingTokens()}},keyup:function(a){this.updatePlaceholder();switch(a.keyCode){case 9:case 13:case 27:case 38:case 40:break;case 8:this.searchInput.val()?this.search():this.dropdownHide();break;default:this.searchInput.val()&&this.search()}},search:function(){var a=this,c=1;if(0<this.options.maxElements&&b("li.Token",this.tokensContainer).length>=
this.options.maxElements||this.searchInput.val().length<this.options.searchMinLength)return!1;if("select"==this.options.datas){var d=!1,e=RegExp(this.searchInput.val().replace(/[-[\]{}()*+?.,\\^$|#\s]/g,"\\$&"),"i");this.dropdownReset();b("option",this.select).not(":selected, :disabled").each(function(){if(c<=a.options.nbDropdownElements)e.test(b(this).html())&&(a.dropdownAddItem(b(this).attr("value"),b(this).html()),d=!0,c++);else return!1});d?(b("li:first",this.dropdown).addClass("Hover"),this.dropdownShow()):
this.dropdownHide()}else this.debounce(function(){b.ajax({url:a.options.datas,data:a.options.searchParam+"="+a.searchInput.val(),dataType:a.options.dataType,success:function(d){if(d&&(a.options.parseData&&(d=a.options.parseData(d)),a.dropdownReset(),b.each(d,function(b,d){if(c<=a.options.nbDropdownElements){var e;d[a.options.htmlField]&&(e=d[a.options.htmlField]);a.dropdownAddItem(d[a.options.valueField],d[a.options.textField],e);c++}else return!1}),b("li",a.dropdown).length))return b("li:first",
a.dropdown).addClass("Hover"),a.dropdownShow(),!0;a.dropdownHide()},error:function(b,c){a.options.onAjaxError(a,b,c)}})},this.options.debounce)},debounce:function(a,b){var d=this,e=arguments;f&&clearTimeout(f);f=setTimeout(function(){a.apply(d,e);f=null},b||this.options.debounce)},tokenAdd:function(a,c,d){a=this.escape(a);if(void 0==a||""==a)return this;c=c||a;d=d||!1;var e;e=this.options.formatTokenHTML?this.options.formatTokenHTML(a,c):"<span>"+c+"</span>";if(!1===e||0<this.options.maxElements&&
b("li.Token",this.tokensContainer).length>=this.options.maxElements)return this.resetSearchInput(),this;var g=this,f=b("<a />").addClass("Close").html("&#215;").on("click",function(b){b.stopImmediatePropagation();g.tokenRemove(a)});if(b('option[value="'+a+'"]',this.select).length)b('option[value="'+a+'"]',this.select).attr("selected",!0).prop("selected",!0);else if(this.options.newElements||!this.options.newElements&&0<b('li[data-value="'+a+'"]',this.dropdown).length){var h=b("<option />").attr("selected",
