(function(g,n){function E(r,a,c,f,d){function h(){I=1<g.devicePixelRatio;c=k(c);0<=a.delay&&setTimeout(function(){q(!0)},a.delay);if(0>a.delay||a.combined)f.e=N(a.throttle,function(b){"resize"===b.type&&(A=B=-1);q(b.all)}),f.a=function(b){b=k(b);c.push.apply(c,b)},f.g=function(){return c=e(c).filter(function(){return!e(this).data(a.loadedName)})},f.f=function(b){for(var a=0;a<b.length;a++){var e=c.filter(function(){return this===b[a]});e.length&&q(!1,e)}},q(),e(a.appendScroll).on("scroll."+d+" resize."+
...
!l.attr(u)?l.attr(u,c):h===v||!J||l.css(w)&&"none"!==l.css(w)||l.css(w,"url('"+J+"')")}return b}function q(b,f){if(c.length){for(var d=f||c,k=!1,h=a.imageBase||"",g=a.srcsetAttribute,n=a.handledName,p=0;p<d.length;p++)if(b||f||E(d[p])){var m=e(d[p]),l=d[p].tagName.toLowerCase(),q=m.attr(a.attribute),s=m.attr(a.imageBaseAttribute)||h,K=m.attr(a.loaderAttribute);m.data(n)||a.visibleOnly&&!m.is(":visible")||!((q||m.attr(g))&&(l===v&&(s+q!==m.attr(u)||m.attr(g)!==m.attr(C))||l!==v&&s+q!==m.css(w))||K)||
(k=!0,m.data(n,!0),z(m,l,s,K))}k&&(c=e(c).filter(function(){return!e(this).data(n)}))}else a.autoDestroy&&r.destroy()}function z(b,c,f,d){++F;var k=function(){s("onError",b);G();k=e.noop};s("beforeLoad",b);var h=a.attribute,r=a.srcsetAttribute,p=a.sizesAttribute,m=a.retinaAttribute,l=a.removeAttribute,g=a.loadedName,n=b.attr(m);if(d){var q=function(){l&&b.removeAttr(a.loaderAttribute);b.data(g,!0);s(L,b);setTimeout(G,1);q=e.noop};b.off(x).one(x,k).one(y,q);s(d,b,function(a){a?(b.off(y),q()):(b.off(x),
k())})||b.trigger(x)}else{var t=e(new Image);t.one(x,k).one(y,function(){b.hide();c===v?b.attr(D,t.attr(D)).attr(C,t.attr(C)).attr(u,t.attr(u)):b.css(w,"url('"+t.attr(u)+"')");b[a.effect](a.effectTime);l&&(b.removeAttr(h+" "+r+" "+m+" "+a.imageBaseAttribute),p!==D&&b.removeAttr(p));b.data(g,!0);s(L,b);t.remove();G()});d=(I&&n?n:b.attr(h))||"";t.attr(D,b.attr(p)).attr(C,b.attr(r)).attr(u,d?f+d:null);t.complete&&t.trigger(y)}}function E(b){var c=b.getBoundingClientRect();b=a.scrollDirection;var d=a.threshold,
