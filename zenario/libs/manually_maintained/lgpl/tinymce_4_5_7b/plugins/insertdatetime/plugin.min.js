...
function(){f(c.getParam("insertdatetime_timeformat",c.translate("%H:%M:%S")))});c.addButton("insertdatetime",{type:"splitbutton",title:"Insert date/time",onclick:function(){f(k||h)},menu:g});tinymce.each(c.settings.insertdatetime_formats||["%H:%M:%S","%Y-%m-%d","%I:%M:%S %p","%D"],function(a){h||(h=a);g.push({text:e(a),onclick:function(){k=a;f(a)}})});c.addMenuItem("insertdatetime",{icon:"date",text:"Date/time",menu:g,context:"insert"})});
