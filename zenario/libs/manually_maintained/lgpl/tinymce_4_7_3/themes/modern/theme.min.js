...
a.title.toLowerCase().indexOf(d)});return 1===h.length&&h[0].title===a?[]:h},m=function(a,b,c,e){var f=function(h){var f=d.find(c),f=k(h,f,e,b);a.showAutoComplete(f,h)};a.on("autocomplete",function(){f(a.value())});a.on("selectitem",function(b){b=b.value;a.value(b.url);var c;c=b.title;c=c.raw?c.raw:c;"image"===e?a.fire("change",{meta:{alt:c,attach:b.attach}}):a.fire("change",{meta:{text:c,attach:b.attach}});a.focus()});a.on("click",function(b){0===a.value().length&&"INPUT"===b.target.nodeName&&f("")});
a.on("PostRender",function(){a.getRoot().on("submit",function(b){if(!b.isDefaultPrevented()){b=a.value();var c=h[e];/^https?/.test(b)&&(c?-1===g.indexOf(c,b)&&(h[e]=c.slice(0,5).concat(b)):h[e]=[b])}})})},r=function(a,b,c){var d=b.filepicker_validator_handler;if(d){var h=function(b){0===b.length?a.statusLevel("none"):d({url:b,type:c},function(b){var c;c=b.status;b=b.message;c="valid"===c?{status:"ok",message:b}:"unknown"===c?{status:"warn",message:b}:"invalid"===c?{status:"warn",message:b}:{status:"none",
message:""};a.statusMessage(c.message);a.statusLevel(c.status)})};a.state.on("change:value",function(a){h(a.value)})}};return a.extend({Statics:{clearHistory:function(){h={}}},init:function(a){var d=this,h=e.tinymce?e.tinymce.activeEditor:c.activeEditor,f=h.settings,g,q,n,l=a.filetype;a.spellcheck=!1;(n=f.file_picker_types||f.file_browser_callback_types)&&(n=b.makeMap(n,/[, ]/));if(!n||n[l])q=f.file_picker_callback,!q||n&&!n[l]?(q=f.file_browser_callback,!q||n&&!n[l]||(g=function(){q(d.getEl("inp").id,
d.value(),l,e)})):g=function(){var a=d.fire("beforecall").meta,a=b.extend({filetype:l},a);q.call(h,function(a,b){d.value(a).fire("change",{meta:b})},d.value(),a)};g&&(a.icon="browse",a.onaction=g);d._super(a);m(d,f,h.getBody(),l);r(d,f,l)}})});k("tinymce.ui.FitLayout",["tinymce.ui.AbsoluteLayout"],function(g){return g.extend({recalc:function(f){var e=f.layoutRect(),d=f.paddingBox;f.items().filter(":visible").each(function(c){c.layoutRect({x:d.left,y:d.top,w:e.innerW-d.right-d.left,h:e.innerH-d.top-
d.bottom});c.recalc&&c.recalc()})}})});k("tinymce.ui.FlexLayout",["tinymce.ui.AbsoluteLayout"],function(g){return g.extend({recalc:function(f){var e,d,c,a,b,h,g,n,p,l,k,v,m,r=[],s,u,t,y,x,z,B,A,H,K,C,E,F,G,D,I,L,M,N=Math.max,Q=Math.min;c=f.items().filter(":visible");a=f.layoutRect();b=f.paddingBox;h=f.settings;e=f.isRtl()?h.direction||"row-reversed":h.direction;g=h.align;n=f.isRtl()?h.pack||"end":h.pack;h=h.spacing||0;if("row-reversed"==e||"column-reverse"==e)c=c.set(c.toArray().reverse()),e=e.split("-")[0];
"column"==e?(x="y",t="h",y="minH",z="maxH",A="innerH",B="top",H="deltaH",K="contentH",D="left",F="w",C="x",E="innerW",G="minW",I="right",L="deltaW",M="contentW"):(x="x",t="w",y="minW",z="maxW",A="innerW",B="left",H="deltaW",K="contentW",D="top",F="h",C="y",E="innerH",G="minH",I="bottom",L="deltaH",M="contentH");l=a[A]-b[B]-b[B];e=u=p=0;for(d=c.length;e<d;e++)k=c[e],v=k.layoutRect(),m=k.settings,m=m.flex,l-=e<d-1?h:0,0<m&&(p+=m,v[z]&&r.push(k),v.flex=m),l-=v[y],m=b[D]+v[G]+b[I],m>u&&(u=m);s={};s[y]=
0>l?a[y]-l+a[H]:a[A]-l+a[H];s[G]=u+a[L];s[K]=a[A]-l;s[M]=u;s.minW=Q(s.minW,a.maxW);s.minH=Q(s.minH,a.maxH);s.minW=N(s.minW,a.startMinWidth);s.minH=N(s.minH,a.startMinHeight);if(!a.autoResize||s.minW==a.minW&&s.minH==a.minH){f=l/p;e=0;for(d=r.length;e<d;e++)k=r[e],v=k.layoutRect(),k=v[z],m=v[y]+v.flex*f,m>k?(l-=v[z]-v[y],p-=v.flex,v.flex=0,v.maxFlexSize=k):v.maxFlexSize=0;f=l/p;r=b[B];s={};0===p&&("end"==n?r=l+b[B]:"center"==n?(r=Math.round(a[A]/2-(a[A]-l)/2)+b[B],0>r&&(r=b[B])):"justify"==n&&(r=b[B],
h=Math.floor(l/(c.length-1))));s[C]=b[D];e=0;for(d=c.length;e<d;e++)k=c[e],v=k.layoutRect(),m=v.maxFlexSize||v[y],"center"===g?s[C]=Math.round(a[E]/2-v[F]/2):"stretch"===g?(s[F]=N(v[G]||0,a[E]-b[D]-b[I]),s[C]=b[D]):"end"===g&&(s[C]=a[E]-v[F]-b.top),0<v.flex&&(m+=v.flex*f),s[t]=m,s[x]=r,k.layoutRect(s),k.recalc&&k.recalc(),r+=m+h}else if(s.w=s.minW,s.h=s.minH,f.layoutRect(s),this.recalc(f),null===f._lastRect&&(c=f.parent()))c._lastRect=null,c.recalc()}})});k("tinymce.ui.FlowLayout",["tinymce.ui.Layout"],
function(g){return g.extend({Defaults:{containerClass:"flow-layout",controlClass:"flow-layout-item",endClass:"break"},recalc:function(f){f.items().filter(":visible").each(function(e){e.recalc&&e.recalc()})},isNative:function(){return!0}})});k("ephox.sugar.impl.ClosestOrAncestor",["ephox.katamari.api.Type","ephox.katamari.api.Option"],function(g,f){return function(e,d,c,a,b){return e(c,a)?f.some(c):g.isFunction(b)&&b(c)?f.none():d(c,a,b)}});k("ephox.sugar.api.search.PredicateFind","ephox.katamari.api.Type ephox.katamari.api.Arr ephox.katamari.api.Fun ephox.katamari.api.Option ephox.sugar.api.node.Body ephox.sugar.api.dom.Compare ephox.sugar.api.node.Element ephox.sugar.impl.ClosestOrAncestor".split(" "),
function(g,f,e,d,c,a,b,h){var q=function(a,c,h){a=a.dom();for(h=g.isFunction(h)?h:e.constant(!1);a.parentNode;){a=a.parentNode;var f=b.fromDom(a);if(c(f))return d.some(f);if(h(f))break}return d.none()},n=function(a,c){return f.find(a.dom().childNodes,e.compose(c,b.fromDom)).map(b.fromDom)},k=function(a,c){var h=function(a){for(var e=0;e<a.childNodes.length;e++){if(c(b.fromDom(a.childNodes[e])))return d.some(b.fromDom(a.childNodes[e]));var f=h(a.childNodes[e]);if(f.isSome())return f}return d.none()};
return h(a.dom())};return{first:function(a){return k(c.body(),a)},ancestor:q,closest:function(a,b,c){return h(function(a){return b(a)},q,a,b,c)},sibling:function(c,h){var e=c.dom();return e.parentNode?n(b.fromDom(e.parentNode),function(b){return!a.eq(c,b)&&h(b)}):d.none()},child:n,descendant:k}});k("ephox.sugar.api.search.SelectorFind",["ephox.sugar.api.search.PredicateFind","ephox.sugar.api.search.Selectors","ephox.sugar.impl.ClosestOrAncestor"],function(g,f,e){var d=function(c,a,b){return g.ancestor(c,
function(b){return f.is(b,a)},b)};return{first:function(c){return f.one(c)},ancestor:d,sibling:function(c,a){return g.sibling(c,function(b){return f.is(b,a)})},child:function(c,a){return g.child(c,function(b){return f.is(b,a)})},descendant:function(c,a){return f.one(a,c)},closest:function(c,a,b){return e(f.is,d,c,a,b)}}});k("tinymce.ui.editorui.FormatUtils",[],function(){return{toggleFormat:function(g,f){return function(){g.execCommand("mceToggleFormat",!1,f)}},postRenderFormat:function(g,f){return function(){var e=
this;if(g.formatter)g.formatter.formatChanged(f,function(d){e.active(d)});else g.on("init",function(){g.formatter.formatChanged(f,function(d){e.active(d)})})}}}});k("tinymce.ui.editorui.Align",["tinymce.core.util.Tools","tinymce.ui.editorui.FormatUtils"],function(g,f){return{register:function(e){e.addMenuItem("align",{text:"Align",menu:[{text:"Left",icon:"alignleft",onclick:f.toggleFormat(e,"alignleft")},{text:"Center",icon:"aligncenter",onclick:f.toggleFormat(e,"aligncenter")},{text:"Right",icon:"alignright",
onclick:f.toggleFormat(e,"alignright")},{text:"Justify",icon:"alignjustify",onclick:f.toggleFormat(e,"alignjustify")}]});g.each({alignleft:["Align left","JustifyLeft"],aligncenter:["Align center","JustifyCenter"],alignright:["Align right","JustifyRight"],alignjustify:["Justify","JustifyFull"],alignnone:["No alignment","JustifyNone"]},function(d,c){e.addButton(c,{active:!1,tooltip:d[0],cmd:d[1],onPostRender:f.postRenderFormat(e,c)})})}}});k("tinymce.ui.fmt.FontInfo",["ephox.katamari.api.Fun","ephox.katamari.api.Option",
"ephox.sugar.api.node.Element","ephox.sugar.api.node.Node","tinymce.core.dom.DOMUtils"],function(g,f,e,d,c){var a=function(a,b){return f.from(c.DOM.getStyle(b,a,!0))},b=function(b){return function(c,g){return f.from(g).map(e.fromDom).filter(d.isElement).bind(function(d){var e;a:{for(e=d.dom();e!==c;){if(e.style[b]){e=e.style[b];e=""!==e?f.some(e):f.none();break a}e=e.parentNode}e=f.none()}return e.or(a(b,d.dom()))}).getOr("")}};return{getFontSize:b("fontSize"),getFontFamily:g.compose(function(a){return a.replace(/[\'\"]/g,
"").replace(/,\s+/g,",")},b("fontFamily")),toPt:function(a,b){if(/[0-9.]+px$/.test(a)){var c=72*parseInt(a,10)/96,d=Math.pow(10,b||0);return Math.round(c*d)/d+"pt"}return a}}});k("tinymce.ui.editorui.FontSelect",["tinymce.core.util.Tools","tinymce.ui.fmt.FontInfo"],function(g,f){var e=function(a,b){var c;g.each(a,function(a){a.value.toLowerCase()===b.toLowerCase()&&(c=a.value)});g.each(a,function(a){c||(a.value?a.value.split(",")[0]:"").toLowerCase()!==(b?b.split(",")[0]:"").toLowerCase()||(c=a.value)});
return c},d=function(a,b){return function(){var c=this;a.on("init nodeChange",function(d){d=f.getFontFamily(a.getBody(),d.element);var g=e(b,d);c.value(g?g:null);!g&&d&&c.text(d?d.split(",")[0]:"")})}},c=function(a){a=a.replace(/;$/,"").split(";");for(var b=a.length;b--;)a[b]=a[b].split("=");return a},a=function(a){a=c(a.settings.font_formats||"Andale Mono=andale mono,monospace;Arial=arial,helvetica,sans-serif;Arial Black=arial black,sans-serif;Book Antiqua=book antiqua,palatino,serif;Comic Sans MS=comic sans ms,sans-serif;Courier New=courier new,courier,monospace;Georgia=georgia,palatino,serif;Helvetica=helvetica,arial,sans-serif;Impact=impact,sans-serif;Symbol=symbol;Tahoma=tahoma,arial,helvetica,sans-serif;Terminal=terminal,monaco,monospace;Times New Roman=times new roman,times,serif;Trebuchet MS=trebuchet ms,geneva,sans-serif;Verdana=verdana,geneva,sans-serif;Webdings=webdings;Wingdings=wingdings,zapf dingbats");
