(function(e){"function"===typeof define&&define.amd?define(["jquery","jquery.ui.widget"],e):"object"===typeof exports?e(require("jquery"),require("./vendor/jquery.ui.widget")):e(window.jQuery)})(function(e){function x(a){var b="dragover"===a;return function(c){c.dataTransfer=c.originalEvent&&c.originalEvent.dataTransfer;var d=c.dataTransfer;d&&-1!==e.inArray("Files",d.types)&&!1!==this._trigger(a,e.Event(a,{delegatedEvent:c}))&&(c.preventDefault(),b&&(d.dropEffect="copy"))}}e.support.fileInput=!(RegExp("(Android (1\\.[0156]|2\\.[01]))|(Windows Phone (OS 7|8\\.0))|(XBLWP)|(ZuneWP)|(WPDesktop)|(w(eb)?OSBrowser)|(webOS)|(Kindle/(1\\.0|2\\.[05]|3\\.0))").test(window.navigator.userAgent)||
...
e.type(a.paramName)?a.paramName[0]:a.paramName;a.headers=e.extend({},a.headers);a.contentRange&&(a.headers["Content-Range"]=a.contentRange);f&&!a.blob&&this._isInstanceOf("File",d)||(a.headers["Content-Disposition"]='attachment; filename="'+encodeURI(d.name)+'"');f?e.support.xhrFormDataFileUpload&&(a.postMessage?(c=this._getFormData(a),a.blob?c.push({name:g,value:a.blob}):e.each(a.files,function(m,h){c.push({name:"array"===e.type(a.paramName)&&a.paramName[m]||g,value:h})})):(b._isInstanceOf("FormData",
a.formData)?c=a.formData:(c=new FormData,e.each(this._getFormData(a),function(m,h){c.append(h.name,h.value)})),a.blob?c.append(g,a.blob,d.name):e.each(a.files,function(m,h){(b._isInstanceOf("File",h)||b._isInstanceOf("Blob",h))&&c.append("array"===e.type(a.paramName)&&a.paramName[m]||g,h,h.uploadName||h.name)})),a.data=c):(a.contentType=d.type||"application/octet-stream",a.data=a.blob||d);a.blob=null},_initIframeSettings:function(a){var b=e("<a></a>").prop("href",a.url).prop("host");a.dataType="iframe "+
...
null,d)},_onSend:function(a,b){b.submit||this._addConvenienceMethods(a,b);var c=this,d,f,g,m,h=c._getAJAXSettings(b),n=function(){c._sending+=1;h._bitrateTimer=new c._BitrateTimer;return d=d||((f||!1===c._trigger("send",e.Event("send",{delegatedEvent:a}),h))&&c._getXHRPromise(!1,h.context,f)||c._chunkedUpload(h)||e.ajax(h)).done(function(l,p,q){c._onDone(l,p,q,h)}).fail(function(l,p,q){c._onFail(l,p,q,h)}).always(function(l,p,q){c._onAlways(l,p,q,h);--c._sending;--c._active;if(h.limitConcurrentUploads&&
h.limitConcurrentUploads>c._sending)for(l=c._slots.shift();l;){if("pending"===c._getDeferredState(l)){l.resolve();break}l=c._slots.shift()}0===c._active&&c._trigger("stop")})};this._beforeSend(a,h);return this.options.sequentialUploads||this.options.limitConcurrentUploads&&this.options.limitConcurrentUploads<=this._sending?(1<this.options.limitConcurrentUploads?(g=e.Deferred(),this._slots.push(g),m=g.pipe(n)):m=this._sequence=this._sequence.pipe(n,n),m.abort=function(){f=[void 0,"abort","abort"];
return d?d.abort():(g&&g.rejectWith(h.context,f),n())},this._enhancePromise(m)):n()},_onAdd:function(a,b){var c=this,d=!0,f=e.extend({},this.options,b),g=b.files,m=g.length,h=f.limitMultiFileUploads,n=f.limitMultiFileUploadSize,l=f.limitMultiFileUploadSizeOverhead,p=0,q=this._getParamName(f),k=0;if(!m)return!1;n&&void 0===g[0].size&&(n=void 0);if((f.singleFileUploads||h||n)&&this._isXHRUpload(f))if(f.singleFileUploads||n||!h)if(!f.singleFileUploads&&n){var u=[];var r=[];for(f=0;f<m;f+=1)if(p+=g[f].size+
l,f+1===m||p+g[f+1].size+l>n||h&&f+1-k>=h)u.push(g.slice(k,f+1)),p=q.slice(k,f+1),p.length||(p=q),r.push(p),k=f+1,p=0}else r=q;else for(u=[],r=[],f=0;f<m;f+=h)u.push(g.slice(f,f+h)),p=q.slice(f,f+h),p.length||(p=q),r.push(p);else u=[g],r=[q];b.originalFiles=g;e.each(u||g,function(v,t){var w=e.extend({},b);w.files=u?t:[t];w.paramName=r[v];c._initResponseObject(w);c._initProgressObject(w);c._addConvenienceMethods(a,w);return d=c._trigger("add",e.Event("add",{delegatedEvent:a}),w)});return d},_replaceFileInput:function(a){var b=
a.fileInput,c=b.clone(!0),d=b.is(document.activeElement);a.fileInputClone=c;e("<form></form>").append(c)[0].reset();b.after(c).detach();d&&c.focus();e.cleanData(b.unbind("remove"));this.options.fileInput=this.options.fileInput.map(function(f,g){return g===b[0]?c[0]:g});b[0]===this.element[0]&&(this.element=c)},_handleFileTreeEntry:function(a,b){var c=this,d=e.Deferred(),f=function(l){l&&!l.entry&&(l.entry=a);d.resolve([l])},g=function(l){c._handleFileTreeEntries(l,b+a.name+"/").done(function(p){d.resolve(p)}).fail(f)},
m=function(){h.readEntries(function(l){l.length?(n=n.concat(l),m()):g(n)},f)},h,n=[];b=b||"";a.isFile?a._file?(a._file.relativePath=b,d.resolve(a._file)):a.file(function(l){l.relativePath=b;d.resolve(l)},f):a.isDirectory?(h=a.createReader(),m()):d.resolve([]);return d.promise()},_handleFileTreeEntries:function(a,b){var c=this;return e.when.apply(e,e.map(a,function(d){return c._handleFileTreeEntry(d,b)})).pipe(function(){return Array.prototype.concat.apply([],arguments)})},_getDroppedFiles:function(a){a=
a||{};var b=a.items;return b&&b.length&&(b[0].webkitGetAsEntry||b[0].getAsEntry)?this._handleFileTreeEntries(e.map(b,function(c){var d;if(c.webkitGetAsEntry){if(d=c.webkitGetAsEntry())d._file=c.getAsFile();return d}return c.getAsEntry()})):e.Deferred().resolve(e.makeArray(a.files)).promise()},_getSingleFileInputFiles:function(a){a=e(a);var b=a.prop("webkitEntries")||a.prop("entries");if(b&&b.length)return this._handleFileTreeEntries(b);b=e.makeArray(a.prop("files"));if(b.length)void 0===b[0].name&&
