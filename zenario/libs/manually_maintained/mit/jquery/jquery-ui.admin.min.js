...
$.ui.plugin.add("draggable","snap",{start:function(a,b,c){a=c.options;c.snapElements=[];$(a.snap.constructor!==String?a.snap.items||":data(ui-draggable)":a.snap).each(function(){var a=$(this),b=a.offset();this!==c.element[0]&&c.snapElements.push({item:this,width:a.outerWidth(),height:a.outerHeight(),top:b.top,left:b.left})})},drag:function(a,b,c){var e,d,f,g,k,m,l,p,h,n,s=c.options,q=s.snapTolerance,r=b.offset.left,t=r+c.helperProportions.width,u=b.offset.top,v=u+c.helperProportions.height;for(h=
c.snapElements.length-1;0<=h;h--)k=c.snapElements[h].left-c.margins.left,m=k+c.snapElements[h].width,l=c.snapElements[h].top-c.margins.top,p=l+c.snapElements[h].height,t<k-q||r>m+q||v<l-q||u>p+q||!$.contains(c.snapElements[h].item.ownerDocument,c.snapElements[h].item)?(c.snapElements[h].snapping&&c.options.snap.release&&c.options.snap.release.call(c.element,a,$.extend(c._uiHash(),{snapItem:c.snapElements[h].item})),c.snapElements[h].snapping=!1):("inner"!==s.snapMode&&(e=Math.abs(l-v)<=q,d=Math.abs(p-
u)<=q,f=Math.abs(k-t)<=q,g=Math.abs(m-r)<=q,e&&(b.position.top=c._convertPositionTo("relative",{top:l-c.helperProportions.height,left:0}).top),d&&(b.position.top=c._convertPositionTo("relative",{top:p,left:0}).top),f&&(b.position.left=c._convertPositionTo("relative",{top:0,left:k-c.helperProportions.width}).left),g&&(b.position.left=c._convertPositionTo("relative",{top:0,left:m}).left)),n=e||d||f||g,"outer"!==s.snapMode&&(e=Math.abs(l-u)<=q,d=Math.abs(p-v)<=q,f=Math.abs(k-r)<=q,g=Math.abs(m-t)<=q,
e&&(b.position.top=c._convertPositionTo("relative",{top:l,left:0}).top),d&&(b.position.top=c._convertPositionTo("relative",{top:p-c.helperProportions.height,left:0}).top),f&&(b.position.left=c._convertPositionTo("relative",{top:0,left:k}).left),g&&(b.position.left=c._convertPositionTo("relative",{top:0,left:m-c.helperProportions.width}).left)),!c.snapElements[h].snapping&&(e||d||f||g||n)&&c.options.snap.snap&&c.options.snap.snap.call(c.element,a,$.extend(c._uiHash(),{snapItem:c.snapElements[h].item})),
c.snapElements[h].snapping=e||d||f||g||n)}});$.ui.plugin.add("draggable","stack",{start:function(a,b,c){var e;a=$.makeArray($(c.options.stack)).sort(function(a,c){return(parseInt($(a).css("zIndex"),10)||0)-(parseInt($(c).css("zIndex"),10)||0)});a.length&&(e=parseInt($(a[0]).css("zIndex"),10)||0,$(a).each(function(a){$(this).css("zIndex",e+a)}),this.css("zIndex",e+a.length))}});
...
$.ui.intersect=function(){return function(a,b,c,e){if(!b.offset)return!1;var d=(a.positionAbs||a.position.absolute).left+a.margins.left,f=(a.positionAbs||a.position.absolute).top+a.margins.top,g=d+a.helperProportions.width,k=f+a.helperProportions.height,m=b.offset.left,l=b.offset.top,p=m+b.proportions().width,h=l+b.proportions().height;switch(c){case "fit":return m<=d&&g<=p&&l<=f&&k<=h;case "intersect":return m<d+a.helperProportions.width/2&&g-a.helperProportions.width/2<p&&l<f+a.helperProportions.height/
2&&k-a.helperProportions.height/2<h;case "pointer":a=e.pageY;c=b.proportions().height;if(l=a>=l&&a<l+c)e=e.pageX,b=b.proportions().width,l=e>=m&&e<m+b;return l;case "touch":return(f>=l&&f<=h||k>=l&&k<=h||f<l&&k>h)&&(d>=m&&d<=p||g>=m&&g<=p||d<m&&g>p);default:return!1}}}();
...
this._renderAxis=function(a){var c,b,d;a=a||this.element;for(c in this.handles){if(this.handles[c].constructor===String)this.handles[c]=this.element.children(this.handles[c]).first().show();else if(this.handles[c].jquery||this.handles[c].nodeType)this.handles[c]=$(this.handles[c]),this._on(this.handles[c],{mousedown:f._mouseDown});this.elementIsWrapper&&this.originalElement[0].nodeName.match(/^(textarea|input|select|button)$/i)&&(b=$(this.handles[c],this.element),d=/sw|ne|nw|se|n|s/.test(c)?b.outerHeight():
b.outerWidth(),b=["padding",/ne|nw|n/.test(c)?"Top":/se|sw|s/.test(c)?"Bottom":/^e$/.test(c)?"Right":"Left"].join(""),a.css(b,d),this._proportionallyResize());this._handles=this._handles.add(this.handles[c])}};this._renderAxis(this.element);this._handles=this._handles.add(this.element.find(".ui-resizable-handle"));this._handles.disableSelection();this._handles.mouseover(function(){f.resizing||(this.className&&(e=this.className.match(/ui-resizable-(se|sw|ne|nw|n|e|s|w)/i)),f.axis=e&&e[1]?e[1]:"se")});
...
b.left+(c.width-a.width));return a},_respectSize:function(a){var b=this._vBoundaries,c=this.axis,e=this._isNumber(a.width)&&b.maxWidth&&b.maxWidth<a.width,d=this._isNumber(a.height)&&b.maxHeight&&b.maxHeight<a.height,f=this._isNumber(a.width)&&b.minWidth&&b.minWidth>a.width,g=this._isNumber(a.height)&&b.minHeight&&b.minHeight>a.height,k=this.originalPosition.left+this.originalSize.width,m=this.position.top+this.size.height,l=/sw|nw|w/.test(c),c=/nw|ne|n/.test(c);f&&(a.width=b.minWidth);g&&(a.height=
b.minHeight);e&&(a.width=b.maxWidth);d&&(a.height=b.maxHeight);f&&l&&(a.left=k-b.minWidth);e&&l&&(a.left=k-b.maxWidth);g&&c&&(a.top=m-b.minHeight);d&&c&&(a.top=m-b.maxHeight);a.width||a.height||a.left||!a.top?a.width||a.height||a.top||!a.left||(a.left=null):a.top=null;return a},_getPaddingPlusBorderDimensions:function(a){var b=0,c=[],e=[a.css("borderTopWidth"),a.css("borderRightWidth"),a.css("borderBottomWidth"),a.css("borderLeftWidth")];for(a=[a.css("paddingTop"),a.css("paddingRight"),a.css("paddingBottom"),
...
e=d.position;a=d._aspectRatio||a.shiftKey;var f={top:0,left:0},g=d.containerElement,k=!0;g[0]!==document&&/static/.test(g.css("position"))&&(f=c);e.left<(d._helper?c.left:0)&&(d.size.width+=d._helper?d.position.left-c.left:d.position.left-f.left,a&&(d.size.height=d.size.width/d.aspectRatio,k=!1),d.position.left=b.helper?c.left:0);e.top<(d._helper?c.top:0)&&(d.size.height+=d._helper?d.position.top-c.top:d.position.top,a&&(d.size.width=d.size.height*d.aspectRatio,k=!1),d.position.top=d._helper?c.top:
...
$.ui.plugin.add("resizable","grid",{resize:function(){var a,b=$(this).resizable("instance"),c=b.options,e=b.size,d=b.originalSize,f=b.originalPosition,g=b.axis,k="number"===typeof c.grid?[c.grid,c.grid]:c.grid,m=k[0]||1,l=k[1]||1,p=Math.round((e.width-d.width)/m)*m,e=Math.round((e.height-d.height)/l)*l,h=d.width+p,n=d.height+e,s=c.maxWidth&&c.maxWidth<h,q=c.maxHeight&&c.maxHeight<n,r=c.minWidth&&c.minWidth>h,t=c.minHeight&&c.minHeight>n;c.grid=k;r&&(h+=m);t&&(n+=l);s&&(h-=m);q&&(n-=l);if(/^(se|s|e)$/.test(g))b.size.width=
h,b.size.height=n;else if(/^(ne)$/.test(g))b.size.width=h,b.size.height=n,b.position.top=f.top-e;else if(/^(sw)$/.test(g))b.size.width=h,b.size.height=n,b.position.left=f.left-p;else{if(0>=n-l||0>=h-m)a=b._getPaddingPlusBorderDimensions(this);0<n-l?(b.size.height=n,b.position.top=f.top-e):(n=l-a.height,b.size.height=n,b.position.top=f.top+d.height-n);0<h-m?(b.size.width=h,b.position.left=f.left-p):(h=m-a.width,b.size.width=h,b.position.left=f.left+d.width-h)}}});
...
!1}))},_mouseDrag:function(a){this.dragged=!0;if(!this.options.disabled){var b,c=this,e=this.options,d=this.opos[0],f=this.opos[1],g=a.pageX,k=a.pageY;d>g&&(b=g,g=d,d=b);f>k&&(b=k,k=f,f=b);this.helper.css({left:d,top:f,width:g-d,height:k-f});this.selectees.each(function(){var b=$.data(this,"selectable-item"),l=!1;b&&b.element!==c.element[0]&&("touch"===e.tolerance?l=!(b.left>g||b.right<d||b.top>k||b.bottom<f):"fit"===e.tolerance&&(l=b.left>d&&b.right<g&&b.top>f&&b.bottom<k),l?(b.selected&&(b.$element.removeClass("ui-selected"),
...
$(".ui-selecting",this.element[0]).each(function(){var c=$.data(this,"selectable-item");c.$element.removeClass("ui-selecting").addClass("ui-selected");c.selecting=!1;c.selected=!0;c.startselected=!0;b._trigger("selected",a,{selected:c.element})});this._trigger("stop",a);this.helper.remove();return!1}});
