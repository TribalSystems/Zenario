(function(E){E.initForm=function(c,y,C,z,A,Q,R,S,L,T,U,F){function M(a,g){return a.ord<g.ord?-1:a.ord>g.ord?1:0}this.containerId=c;var D=this;F=JSON.parse(F);U&&$("#"+c+"_user_form").effect("shake",{distance:10,times:3,duration:300});$("#"+c+"_user_form form").on("submit",function(){window.onbeforeunload=null});zenario.isPublic||zenario._sP(this,6E5);if(T){if(1<L&&(window.onbeforeunload=function(){return!0},void 0!==zenario_conductor)){var N=this.phrase("Changes you made may not be saved.");zenario_conductor._cOC(c,
function(){return!0},function(a){confirm(N)&&a()},N)}A&&(window.onbeforeunload=null)}if(R)$("#"+c+" .page_switcher li.step").on("click",function(){var a=$(this).data("page");a<=L&&a!=S&&(window.onbeforeunload=null,D.submitForm(c,{target_page:a},!0))});z&&($.colorbox({transition:"none",html:z,escKey:!1,overlayClose:!1,onOpen:function(){var a=get("colorbox");a.className=E.moduleClassName;$(a).hide().fadeIn()}}),zenario._rC());$("#"+c+"_user_form .form_buttons .next.submit").on("click",function(){D.submitForm(c,
{submitForm:!0},!0)});$("#"+c+"_user_form .form_buttons .next:not(.submit)").on("click",function(){D.submitForm(c,{next:!0},!0)});$("#"+c+"_user_form .form_buttons .previous").on("click",function(){D.submitForm(c,{previous:!0},!0)});$("#"+c+" input.saveLater").on("click",function(){var a=$(this).data("message");confirm(a)&&D.submitForm(c,{saveLater:!0})});$("#"+c+"_user_form .field_attachment .remove_attachment").on("click",function(){var a=$(this).data("id"),g={};g["remove_attachment_"+a]=!0;D.submitForm(c,
g)});$("#"+c+"_print_page").on("click",function(){D.printFormPage(c)});var O=$("#"+c+"_fullscreen"),V=$("#"+c+' input[name="inFullScreen"]');O.on("click",function(){$("#ui-datepicker-div").detach().appendTo("#"+c);zenario._eFS($("#"+c)[0])});$(document).on(zenario.fullScreenChangeEvent,function(){var a=zenario._iFS();O.toggle(!a);V.val(a?1:0);$("#"+c+"_form_wrapper").toggleClass("in_fullscreen",a)});A&&zenario._exFuSc();$("#"+c+"_user_form input.jquery_form_datepicker").each(function(a,g){if(g.id){g.value=
$.datepicker.formatDate(zenario.dpf,$.datepicker.parseDate("yy-mm-dd",get(g.id+"__0").value));a={dateFormat:zenario.dpf,altField:"#"+g.id+"__0",altFormat:"yy-mm-dd",showOn:"focus",onClose:function(f,e){get(g.id+"__0");e=$.datepicker.formatDate(zenario.dpf,$.datepicker.parseDate("yy-mm-dd",get(g.id+"__0").value));f&&e?f&&e&&(g.value=e):l.datepicker("setDate",null)}};$(this).data("selectors")&&(a.changeMonth=!0,a.changeYear=!0,a.yearRange="c-100:c+5");$(this).data("no_past_dates")&&(a.minDate=new Date);
$(this).data("no_future_dates")&&(a.maxDate=new Date);var l=$("#"+g.id);l.datepicker(a);$("#"+g.id+"__clear").on("click",function(){l.datepicker("setDate",null)});Q&&zenario._iFS()&&$("#ui-datepicker-div").detach().appendTo("#"+c)}});$("#"+c+"_user_form select.source_field").on("change",function(){E.submitForm(c,{filter:1})});$("#"+c+"_user_form .form_field.visible_on_condition, #"+c+"_user_form .repeat_block.visible_on_condition, .page_switcher li.visible_on_condition").each(function(a,g){var l=
this,f=$(this).data("cfields");$(this).data("id");if(f.length)for(a=0;a<f.length;a++){var e=f[a];(function(d){$("#"+c+"_field_"+e.id+" :input").on("change",function(){for(var b=!0,n=d;n<f.length;n++){var h=f[n],m=$("#"+c+"_field_"+h.id+" :input");if("checkboxes"==h.type){var p=[];m.each(function(){$(this).is(":checked")&&p.push($(this).data("value"))});m=h.value?_._m(h.value.toString().split(","),Number):[];if("AND"==h.operator)b=_.intersection(p,m),b=_.intersection(p,b),b=_._isEq(b,m);else{b=!1;
for(var t=0;t<p.length;t++)if(-1!=m.indexOf(p[t])){b=!0;break}}}else m=m.is(":checkbox")?m.is(":checked"):"radios"==h.type||"centralised_radios"==h.type?m.filter(":checked").val():m.val(),b="string"==typeof h.value&&1<h.value.split(",").length?-1!==h.value.split(",").indexOf(m):!!(""===h.value&&m||""!==h.value&&h.value==m);b=h.invert?!b:b;if(!b||n==f.length-1){$(l).toggle(b);break}}})})(a)}});$("#"+c+"_user_form .form_field.restatement").each(function(a,g){var l=this;if(a=$(this).data("fieldid"))if(a=
...
g&&(g={},g["set_predefined_text_"+a]=!0,D.submitForm(c,g))});$(function(){$("#"+c+"_sortable1, #"+c+"_sortable2").sortable({connectWith:".connectedSortable",update:function(a,g){a=$("#"+c+"_sortable1");a=String(a.sortable("toArray",{attribute:"data-value"}));$("#"+c+"_sortable1_values").val(a);a=$("#"+c+"_sortable2").sortable("toArray",{attribute:"data-value"});$("#"+c+"_sortable2_values").val(a)}}).disableSelection()})};E.printFormPage=function(){var c=$("#"+this.containerId+" .form_fields").html(),
y=window.screenX+$(window).width()/2,C=window.open("","Print-Window","width=300,height=300,left="+y+",top="+window.screenY);C.document.open();C.document.write('<html><body onload="window.print()">'+c+"</body></html>");C.document.close();setTimeout(function(){C.close()},10)};E.buildFieldCalculation=function(c,y,C){for(var z="",A=0;A<y.length;A++)switch(y[A].type){case "static_value":z+=+y[A].value;break;case "field":if("NaN"==y[A].v&&0>=$("#"+c+'_user_form .form_field input[name="field_'+y[A].value+
'"]').length)return!1;C[y[A].value]=y[A].v;z+="[[FIELD_"+y[A].value+"]]";break;case "parentheses_open":z+="(";break;case "parentheses_close":z+=")";break;case "operation_addition":z+="+";break;case "operation_subtraction":z+="-";break;case "operation_multiplication":z+="*";break;case "operation_division":z+="/"}return z};E.submitForm=function(c,y,C){var z=$("#"+c+"_user_form form");C||(zenario.blockScrollToTop=!0);if(y)for(var A in y)z.append('<input type="hidden" name="'+A+'" value="'+y[A]+'"/>');
$("#"+c+"_user_form .field_document_upload, #"+c+"_user_form .field_file_picker").each(function(){$(this).find('input[type="file"]').remove()});z.submit()};E.toggleForm=function(c){$("#"+c).toggleClass("show").toggleClass("hide")}})(zenario_user_forms);
