(function(e){e.initForm=function(a,c,n,b,h,z,v,w,t,x){this.containerId=a;var q=this;$("#"+a+"_user_form form").on("submit",function(){window.onbeforeunload=null});this.startPoking();x&&(1<t&&(window.onbeforeunload=function(){return!0}),h&&(window.onbeforeunload=null));if(v)$("#"+a+" .page_switcher li.step").on("click",function(){var f=$(this).data("page");f<=t&&f!=w&&(window.onbeforeunload=null,q.submitForm(a,c,{target_page:f},!0))});b&&($.colorbox({transition:"none",html:b,escKey:!1,overlayClose:!1,
onOpen:function(){var a=get("colorbox");a.className=e.moduleClassName;$(a).hide().fadeIn()}}),zenario.resizeColorbox());$("#"+a+"_print_page").on("click",function(){q.printFormPage(a)});var r=$("#"+a+"_fullscreen"),y=$("#"+a+' input[name="inFullScreen"]');r.on("click",function(){zenario.enableFullScreen($("#"+a)[0])});$(document).on(zenario.fullScreenChangeEvent,function(){var f=zenario.isFullScreen();r.toggle(!f);y.val(f?1:0);$("#"+a+"_form_wrapper").toggleClass("in_fullscreen",f)});h&&zenario.exitFullScreen();
...
b){var k=$(this).data("cfieldid"),g=$(this).data("cfieldvalue"),e=$(this).data("id");if(k)$("#"+a+"_field_"+k+" :input").on("change",function(){var d;d=$(this).is(":checkbox")?$(this).is(":checked"):$(this).val();!g&&d||g&&g==d?$("#"+a+"_repeat_block_"+e).show():$("#"+a+"_repeat_block_"+e).hide()})});$("#"+a+"_user_form .form_field.restatement").each(function(f,b){var k=this,g=$(this).data("fieldid");if(g&&(g=$("#"+a+'_user_form .form_field :input[name="field_'+g+'"]'),0<g.length))if(g.is("input"))g.on("keyup",
function(){$(k).find(":input").val($(this).val())});else if(g.is("select"))g.on("change",function(){$(k).find(":input").val(""===$(this).val()?"":$(this).find("option:selected").text())})});$("#"+a+"_user_form .form_field.calculated").each(function(f,b){var k=$(this).find("input"),g=$(this).data("prefix"),e=$(this).data("postfix"),d=$("#"+a+"_field_"+$(b).data("id")+"_calculation_code").text();if(d){var d=JSON.parse(d),c={},h="";if(d&&(h=q.buildFieldCalculation(a,d,c))){var n={};for(f in c)d=$("#"+
