...
class zenario_abstract_nest__admin_boxes__slide extends zenario_abstract_nest {
	...
	public function fillAdminBox($path, $settingGroup, &$box, &$fields, &$values) {
		...
		if ($box['key']['usesConductor']) {
			...
			if (!empty($states)) {
				...
				$sql = "
					SELECT 1
					FROM ". DB_PREFIX. "nested_paths AS p
					...
					WHERE p.instance_id = ". (int) $instance['instance_id']. "
					  AND p.to_state IN (". ze\escape::in($states, 'sql'). ")
					  AND p.is_forwards = 1";
				
				$box['key']['breadcrumbsOverridden'] = ze\sql::exists($sql);
				
				foreach ($states as $state) {
					//Check if the slide has a back link set
					$sql = '
						SELECT 1
						FROM '. DB_PREFIX. 'nested_paths AS back
						WHERE back.instance_id = '. (int) $instance['instance_id']. '
						  AND back.from_state = \''. \ze\escape::sql($state). '\'
						  AND back.command = \'back\'
						  AND back.equiv_id = 0
						LIMIT 1';
					$hasBackPathAway = ze\sql::exists($sql);
					
					$sql = '
						SELECT 1
						FROM '. DB_PREFIX. 'nested_paths AS forwards
						WHERE forwards.instance_id = '. (int) $instance['instance_id']. '
						  AND forwards.to_state = \''. \ze\escape::sql($state). '\'
						  AND forwards.command NOT IN (\'back\', \'submit\')
						  AND forwards.equiv_id = 0
						LIMIT 1';
					$hasForwardPathTo = ze\sql::exists($sql);
					
					$box['tabs']['details']['notices']['no_path_back_set']['show'] = $hasForwardPathTo && !$hasBackPathAway;
				}
			}
		}
		...
		if ($box['key']['usesConductor']) {
			if (ze\plugin::setting('show_global_tabs', $box['key']['instanceId'], 0)) {
				if ($box['key']['breadcrumbsOverridden']) {
					$fields['details/slide_label_notices']['notices_below']['appearance'] = [
						'type' => 'information',
						'message' => ze\admin::phrase('The above label will appear on links to this slide (e.g. tabs). It will not appear on breadcrumb-links, as it is overridden by a plugin generating smart breadcrumbs on the slide above this one in the navigation.')
					];
				} else {
					$fields['details/slide_label_notices']['notices_below']['appearance'] = [
						'type' => 'information',
						'message' => ze\admin::phrase('The above label will appear on links to this slide (e.g. breadcrumb links and tab).')
					];
				}
			} else {
				if ($box['key']['breadcrumbsOverridden']) {
					$fields['details/slide_label_notices']['notices_below']['appearance'] = [
						'type' => 'warning',
						'message' => ze\admin::phrase('The above label will not appear on breadcrumb trail links, as it is overridden by a plugin generating smart breadcrumbs on the slide above this one in the navigation.')
					];
				} else {
					$fields['details/slide_label_notices']['notices_below']['appearance'] = [
						'type' => 'information',
						'message' => ze\admin::phrase('The above label will appear on links (e.g. breadcrumb-links) to this slide.')
					];
				}
			}
			...
			switch ($nestType) {
				...
				case 'tabs_and_buttons':
					...
					$fields['details/slide_label_notices']['notices_below']['appearance'] = [
						'type' => 'information',
						'message' => ze\admin::phrase('The above label will appear on links to this slide (e.g. tabs).')
					];
					break;
		...
		if ($box['key']['usesConductor'] && !$box['key']['breadcrumbsOverridden']) {
			$fields['details/slide_label']['onchange'] = "
				zenario.ajax(zenario_ajax_nest.AJAXLink({formatTitleTextAdmin: this.value, htmlescape: true})).after(function(html) {
	...
	public function formatAdminBox($path, $settingGroup, &$box, &$fields, &$values, $changes) {
		if ($box['key']['usesConductor'] && !$box['key']['breadcrumbsOverridden']) {
			$fields['details/slide_label_notices']['pre_field_html'] =
				'<span id="zenario__title_merge_fields__note">'.
					zenario_abstract_nest::formatTitleTextAdmin($values['details/slide_label'], true).
				'</span><br/>';
		}
		
		
		//Check whether the "Show images on tab links" option was selected for the nest,
		//and if we should show an image picker here.
		//For conductors, the "Set a global command to navigate to this slide" also needs to be set
		//for the option to appear.
		$showImageSelector = false;
		$imageSettingIndent = 0;
		...
		if ($box['key']['usesConductor']) {
		...
		} elseif ('accordion' == ze\plugin::setting('animation_library', $box['key']['instanceId'])) {
			...
			switch ($nestType) {
				...
				case 'tabs_and_buttons':
					...
					if (ze\plugin::setting('show_images_on_slide_links', $box['key']['instanceId'], 0)) {
						$showImageSelector = true;
						$imageSettingIndent = 0;
					}
					break;
		...
		if ($showImageSelector) {
			$fields['details/slide_link_image_id']['indent'] = $imageSettingIndent;
			$fields['details/slide_link_image_id']['hidden'] = false;
		} else {
			$fields['details/slide_link_image_id']['hidden'] = true;
		}
	}
	...
	public function saveAdminBox($path, $settingGroup, &$box, &$fields, &$values, $changes) {
		...
		//Check if an image was set. If it was a newly uploaded image we'll need to call
		//the getPathOfUploadInCacheDir() and addToDatabase() functions on it.
		if (empty($fields['details/slide_link_image_id']['hidden'])) {
			
			$slideImageId = $values['details/slide_link_image_id'];
			
			if ($uploadPath = ze\file::getPathOfUploadInCacheDir($slideImageId)) {
				$slideImageId = ze\fileAdm::addToDatabase('image', $uploadPath);
			}
			
			$details['slide_link_image_id'] = $slideImageId;
		}
		...
		//If the image on the slide has changed, we'll need to call the resyncLibraryPluginFiles() function
		//to update the table that tracks where images are used.
		$previousImage = (int) ($fields['details/slide_link_image_id']['value'] ?? 0) ;
		$currentImage = (int) ($values['details/slide_link_image_id'] ?? 0);
		if ($previousImage != $currentImage) {
			ze\contentAdm::resyncLibraryPluginFiles($box['key']['instanceId']);
		}
	}
