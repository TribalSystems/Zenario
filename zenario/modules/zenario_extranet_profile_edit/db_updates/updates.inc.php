...
if (ze\dbAdm::needRevision(36)) {
	...
	foreach ($codes as $phrase => $code) {
		$visitorPhrasesResult = ze\row::query('visitor_phrases', true, array('code' => $code, 'module_class_name' => 'zenario_extranet_profile_edit'));
		while ($visitorPhrase = ze\sql::fetchAssoc($visitorPhrasesResult)) {
			ze\row::update('visitor_phrases', array('code' => $phrase), array('id' => $visitorPhrase['id']));
		}
	}
