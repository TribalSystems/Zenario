...
"click",a.cluster_);google.maps.event.trigger(f,"clusterclick",a.cluster_);if(f.getInfoOnClick()&&f.getMap().getZoom()>=f.getInfoOnClickZoom()){for(var k=a.cluster_.getMarkers(),l=[],g=0;g<k.length;g++)l.push(void 0!==k[g].content&&""!=k[g].content?k[g].content:k[g].title);(g=f.reusableInfoWindow_)||(g=new google.maps.InfoWindow);g.setContent(l.join("<br>"));g.open(f.getMap(),k[0])}else f.getZoomOnClick()&&(h=f.getMaxZoom(),e=a.cluster_.getBounds(),f.getMap().fitBounds(e),setTimeout(function(){f.getMap().fitBounds(e);
