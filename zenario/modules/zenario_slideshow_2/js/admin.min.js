...
zenario_slideshow_2.selectSlide=function(a){if(a&&this.data.slides[a]){this.tempSaveData(this.current);this.highlightSlide(a);var c=this.data.slides[a];"internal"!=c.target_loc||c.content_item_tag_id||(c.content_item_link="Nothing selected");c.rollover_image_id||(c.rollover_image_src_thumbnail_1="zenario/modules/zenario_slideshow_2/images/image-in-slot-placeholder.png",c.r_alt_tag="No image");c.mobile_image_id||(c.mobile_image_src_thumbnail_1="zenario/modules/zenario_slideshow_2/images/image-in-slot-placeholder.png",
c.m_alt_tag="No image");c.open_in_new_window=zenario.engToBoolean(c.open_in_new_window);c.link_to_translation_chain=zenario.engToBoolean(c.link_to_translation_chain);c.use_transition_code=zenario.engToBoolean(c.use_transition_code);c.hidden=zenario.engToBoolean(c.hidden);c.has_tabs=this.tabs;var b=function(a,b){var c=[];for(key in a)a.hasOwnProperty(key)&&(key==b&&(a[key].selected=!0),a[key].value=key,c.push(a[key]));c.sort(d);return c},d=function(a,b){return+a.ord<+b.ord?-1:+a.ord>+b.ord?1:0},e=
...
else $("#zenario_edit_mobile_image").text("Mobile Image (Disabled in properties)").addClass("disabled");$("#zenario_select_link_type").off().on("change",function(){a.selectSlide(a.current)});$("#zenario_select_slide_visibility").off().on("change",function(){a.selectSlide(a.current)});$("#zenario_external_link").off().on("blur",function(){"http://"==this.value&&(this.value="")});$("#zenario_external_link").on("focus",function(){this.value||(this.value="http://")});var c=function(b){a.tempSaveData(a.current);
data={mode:"save_slides",slides:JSON.stringify(a.data.slides),ordinals:a.slideOrderedIds.join(",")};zenario.ajax(zenario.addBasePath(a.AJAXLink),data).after(function(c){a.errors=JSON.parse(c);if(_.isEmpty(a.errors))zenario.refreshPluginSlot(a.slotName,"lookup"),$("#zenario_slideshow_error_display").html("");else{var e="";for(key in a.errors)a.errors.hasOwnProperty(key)&&a.errors[key].forEach(function(a){e+='<div class="error">Slide '+(+key+1)+": "+a+"</div>"});$("#zenario_slideshow_error_display").html(e)}b&&
_.isEmpty(a.errors)&&a.closeImageManager("save")})};$("#zenario_save_continue").off().on("click",function(){c(!1)});$("#zenario_save_close").off().on("click",function(){c(!0)})};zenario_slideshow_2.organizerChangeImage=function(){zenarioA.organizerSelect("zenario_slideshow_2","organizerChangeImageCallback",!1,"zenario__content/panels/image_library","zenario__content/panels/image_library","zenario__content/panels/image_library","zenario__content/panels/image_library",!0)};
...
zenario_slideshow_2.uploadImages=function(){var a=this,c={mode:"file_upload"},b=!zenarioA.canDoHTML5Upload(),d='<input type="file" name="Filedata" accept="image/*"';b&&(c._html5_backwards_compatibility_hack=1,d+=' id="zenario_fallback_fileupload"');b||(d+=" multiple");d+="/>";if(b){var b="",d='<form id="zenario_bc_form" action="'+htmlspecialchars(this.AJAXLink)+'" onsubmit="/*...*/" target="zenario_iframe" method="post" enctype="multipart/form-data">'+d,e;for(e in c)d+='<input type="hidden" value="'+
htmlspecialchars(c[e])+'" name="'+htmlspecialchars(e)+'"/>';b='<input type="button" class="submit_selected" value="'+zenarioA.phrase.upload+'" onclick="get(\'zenario_bc_form\').submit();"/>';b+='<input type="button" class="submit" value="'+zenarioA.phrase.cancel+'"/>';zenarioA.showMessage(d+"</form>",b);zenario.browserIsIE()||$("#zenario_fallback_fileupload").click()}else d=$(d),d.change(function(){zenarioA.doHTML5Upload(this.files,a.AJAXLink,c,function(b){a.uploadImagesCallback(b)})}),d.click()};
...
zenario_slideshow_2.resize=function(){maxHeight=800;this.sizing&&clearTimeout(this.sizing);if(get("zenario_fbAdminInner")){var a=Math.floor(0.96*$(window).height()-150);a>maxHeight&&(a=maxHeight);0<(a*=1)&&(get("zenario_fbAdminInner").style.height=a+"px",get("zenario_slides").style.height=get("zenario_slide_attributes").style.height=a-60+"px")}this.sizing=setTimeout(zenario_slideshow_2.resize,250)};
