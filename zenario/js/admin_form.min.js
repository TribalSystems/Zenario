...
75,function(){m.browserIsIE()&&c.style.removeAttribute("filter");c.addJQueryElementsToTabAndFocusFirstField()});c.hideShowFields()}});else{this.insertHTML(a,b);var e=this.lastScrollTop;$("#zenario_abtab").clearQueue().show().animate({opacity:1},150,function(){m.browserIsIE()&&c.style.removeAttribute("filter");h||e===k?c.addJQueryElementsToTabAndFocusFirstField():c.addJQueryElementsToTab()});$("#zenario_fbAdminInner").scrollTop(e);this.hideShowFields()}delete this.tuix.shake}else this.insertHTML(a,
b),$("#zenario_abtab").clearQueue().show().animate({opacity:1},150,function(){m.browserIsIE()&&c.style.removeAttribute("filter");c.addJQueryElementsToTabAndFocusFirstField()})};f.drawTabs=function(a){a||(a=this.mtPrefix+"_tab");var b=[],d;for(d in this.sortedTabs)if(m.has(this.sortedTabs,d)){var c=this.sortedTabs[d];this.tuix.tabs[c]&&!q.hidden(this.tuix.tabs[c])&&(this.tuix.tab||(this.tuix.tab=c),b.push({tabId:c,current:this.tuix.tab==c,label:this.tuix.tabs[c].label}))}return q.microTemplate(a,b)};
f.drawFields=function(a){q.clearHTML5UploadFromDragDrop();var b=this.tuix.tab,d="",c="";!this.savedAndContinued(b)&&this.editCancelEnabled(b)&&(c='<div class="zenario_editCancelButton"><input class="submit" type="button" onclick="'+this.globalName+'.changeMode(); return false;" value="'+(this.editModeOn()?C.cancel:C.edit)+'"></div>');var h=x(this.tuix.tabs[b].template,this.mtPrefix+"_current_tab"),e=!1,p,g,k,c={fields:{},rows:[],tabId:b,path:this.path,revert:c,errors:[],notices:{}};if(this.editModeOn()){if(this.tuix.tabs[b].errors)for(p in this.tuix.tabs[b].errors)m.has(this.tuix.tabs[b].errors,
