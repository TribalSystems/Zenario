...
function(){return this.drawFields(cb)};f.animateInTab=function(a,b,d){var c=this,g=this.tuix.shake===h?this.differentTab&&this.errorOnTab(this.tuix.tab):p(this.tuix.shake);if(this.shownTab==this.tuix.tab){if(g&&!d.find("iframe").length)d.effect({effect:"bounce",duration:125,direction:"right",times:2,distance:5,mode:"effect",complete:function(){c.insertHTML(a,b);$("#zenario_abtab").clearQueue().show().animate({opacity:1},75,function(){n.browserIsIE()&&this.style.removeAttribute("filter");c.addJQueryElementsToTabAndFocusFirstField()});
c.hideShowFields()}});else{this.insertHTML(a,b);var e=this.lastScrollTop;$("#zenario_abtab").clearQueue().show().animate({opacity:1},150,function(){n.browserIsIE()&&this.style.removeAttribute("filter");g||e===h?c.addJQueryElementsToTabAndFocusFirstField():c.addJQueryElementsToTab()});$("#zenario_fbAdminInner").scrollTop(e);this.hideShowFields()}delete this.tuix.shake}else this.insertHTML(a,b),$("#zenario_abtab").clearQueue().show().animate({opacity:1},150,function(){n.browserIsIE()&&this.style.removeAttribute("filter");
...
this.globalName+".meMarkChanged('"+s(d)+"', this.value, '"+s(e.value)+"');");r["class"]+="input_"+e.type;if("select"==e.type)e.slider&&(v=!0,c+=this.drawSlider(a,d,e,m,!0)),c+="<select";else if("textarea"==e.type)c+="<textarea";else if("code_editor"==e.type)c+="<div",r["class"]=" zenario_embedded_ace_editor",a.after(function(){var a=ace.edit(d);a.session.setUseSoftTabs(!1);a.setShowPrintMargin(!1);m&&(a.setReadOnly(!0),a.setBehavioursEnabled(!1));p(e.wrap_text)&&a.session.setUseWrapMode(!0);a.session.setOption("useWorker",
!1);if(language=e.language)try{language=language.match(/\.twig\.html$/i)?"ace/mode/twig":language.match(/\./)?ace.require("ace/ext/modelist").getModeForPath(language).mode:"ace/mode/"+language,a.session.setMode(language)}catch(b){console.log("Ace editor could not load that language",language)}});else if("color_picker"==e.type||"colour_picker"==e.type)c+="<input",r["class"]=" zenario_color_picker",a.after(function(){var a=e.color_picker_options||e.colour_picker_options||{};a.disabled=m;a.preferredFormat=
a.preferredFormat||"hex";$(l(d)).spectrum(a)});else if("editor"==e.type){c+="<textarea";q.getSkinDesc();var J;f={script_url:H+q.tinyMCEPath,inline:!1,menubar:!1,statusbar:!1,plugins:"autoresize",document_base_url:h,convert_urls:!1,readonly:!0,inline_styles:!1,allow_events:!0,allow_script_urls:!0,autoresize_max_height:Math.max(Math.floor(0.5*$(L).height()),300),onchange_callback:function(a){B.fieldChange(a.id)},init_instance_callback:function(a){q.enableDragDropUploadInTinyMCE(!1,h,l("row__"+x(a.editorId,
