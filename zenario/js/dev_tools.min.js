...
n){n=a.drillDownIntoSchema(v,n);n.parent&&a.formatRequiredPropertiesInSchema(n.parent);n.tag=v.s("/").pop();n.isRequired=n.parent&&n.parent.requiredProperties&&n.parent.requiredProperties[n.tag];m="";m=n.exact?y._mT("zenario_dev_tools_tooltip",n):(n.internalCMSProperty=a.internalCMSProperties[n.tag])?y._mT("zenario_dev_tools_internal_property_tooltip",n):"";h.html(m)}):(m="",h.html(m)),l=u.row),m?h.show().css("top",x.clientY+20).css("left",x.clientX):h.hide())});B.on("mouseout",function(){h.hide()})};
a.load=function(){A._nDS("loading");if(a.mode&&r&&r[a.mode]&&(a.orgMap||r[a.mode].url)){sourceLib=r[a.mode];if(a.orgMap){var b=J+"zenario/admin/organizer.ajax.php?_debug=1";a.path=""}else b=sourceLib.devToolsURL?sourceLib.devToolsURL+"&_debug=1":sourceLib.url+"&_debug=1",a.path=sourceLib.path;t._a(b,!1,!0).after(function(c){a.focus=c;a.tagPath=a.focus.tag_path||a.path||"";a.orgMap&&(a.map=$.extend(!0,{},sourceLib.map),a.filterNav(a.map),a.filterNav(a.focus.tuix));a.focus.files={};var d,e=a.focus.modules_files_loaded,
g,f={},h;for(d in e)if(p(e,d)){var l=e[d];if(g=l.paths)for(h in g)p(g,h)&&(f[d+"."+h]=!1)}c="zenario/admin/dev_tools/ajax.php?mode="+K(a.mode);f={load_tuix_files:JSON.stringify(f)};t._a(c,f,!0).after(function(m){if(m)for(d in e)if(p(e,d)&&(l=e[d],g=l.paths))for(h in g)p(g,h)&&m[d+"."+h]&&(m[d+"."+h].path=g[h],a.focus.files[d+"."+h]=m[d+"."+h]);a.draw();A._nDS(!1)})})}else alert("Error, could not load!")};a.filterNav=function(b,c,d,e){q(c)||(c=sourceLib.currentTopLevelPath.s("/"),c=c[0]);delete b.branch;
...
b):d.fullPath=d.localPath=b;"zenarioO"==a.mode&&(d.shortPath=sourceLib.shortenPath(d.fullPath));b="";c=d.localPath.s("/");for(var e in c)p(c,e)&&(d.tag=c[e],b+=(b?"/":"")+c[e],d.paths.push({tag:c[e],path:b}),d.displayedPath=b)}d.object.files=a.highlightFilesContainingSelection(b||d.object.path);w("sidebar_inner").innerHTML=y._mT("zenario_dev_tools_sidepanel",d);G.removeClass("sidebarEmpty").addClass("sidebarFull");a.sizePropertyTable();A._to("#sidebar_inner tr[title]",{tooltipClass:"zenario_devtools_tooltip",
position:{my:"right top+2",at:"left center",collision:"flipfit"},show:!1,hide:!1})};a.sizePropertyTable=function(){if(w("properties_table")){var b=$("#properties_table .property_name").width()||0;b=L.innerWidth()-b-20;$("#properties_table .property_desc").width(b);$("#properties_table .property_desc div").width(b)}};a.editorOnChange=function(b){a.editorSetValueByScript||t._aADINS("dev_tools_editor_change",function(){a.validate()},2E3)};a.editorOnSelect=function(b){a.editorSetValueByScript||(w("sidebar_inner").innerHTML=
"",G.addClass("sidebarEmpty").removeClass("sidebarFull"),t._aADINS("dev_tools_editor_select",function(){a.locatePosition()},500))};k.on("change",a.editorOnChange);k.on("changeCursor",a.editorOnSelect);k.on("changeSelection",a.editorOnSelect);k.session.setUseSoftTabs(!1)},window.devTools=function(){},window.editor=ace.edit("editor"));
