...
""));c._sCO(f);b.restoreScrollPosition(a)}});h();l.each(function(a,b){b.id&&(m[b.id]=1*$(b).position().left)})};d.setupReordering=function(a){var d=this,b=d.ordinalColumn();b&&!u(this.searchTerm)&&a.find("#organizer_items_wrapper").sortable({opacity:0.8,cancel:".organizer_slidedown_view,input,textarea,button,select,option",start:function(){d.closeInspectionView();c._dI()},stop:function(l,n){c._eI();var e,f,m="",h={},p=[],q=[],k=a.find("#organizer_items_wrapper").sortable("toArray",{attribute:"data-id"});
for(e=k.length;0<=e;--e)""!==k[e]&&u(k[e])||k.splice(e,1);for(f in k)r(k,f)&&(e=k[f],h[e]=!0);for(f in c.searchedItems)r(c.searchedItems,f)&&(e=c.searchedItems[f],h[e]&&(q.push(e),p.push(d.tuix.items[e][b])));h=c._gK();h.ordinals={};for(f in k)r(k,f)&&(e=k[f],e!=q[f]&&(m+=(m?",":"")+e,h.ordinals[e]=p[f]));h.reorder=!0;h.id=m;h.dropped_item=$(n.item).data("id");e="zenario/ajax.php?__pluginClassName__="+d.tuix.reorder.class_name+"&__path__="+c.path+"&method_call=handleOrganizerPanelAJAX";delete x.rev;
$.post(w+e,h,function(){c._r()})}})};d.ordinalColumn=function(){return!t.zenarioOSelectMode&&this.tuix.reorder&&this.tuix.reorder.column}},zenarioO.panelTypes);
