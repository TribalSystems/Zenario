...
e.tuix.link+'zenario/admin/organizer.php?#zenario__languages/panels/salutations" target="_blank">Manage list of salutations</a>'}}};h.validateTUIX=function(d,a,b,c){a.name||(c.tabs[b].fields.name.error="Please enter a name.");if("details"==b&&a.visibility&&"visible_on_condition"==a.visibility)if(!a.visible_condition_field_id)c.tabs[b].fields.visible_condition_field_id.error="Please select a visible on conditional form field.";else if(!a.visible_condition_field_value){var f=this.getItem("field",a.visible_condition_field_id);
!f||"checkbox"!=f.type&&"group"!=f.type||(c.tabs[b].fields.visible_condition_field_value.error="Please select a visible on condition form field value.")}if("field"==d)switch(b){case "details":if("checkbox"==a.type||"group"==a.type)a.label||(c.tabs[b].fields.label.error="Please enter a label for this checkbox.");else if("repeat_start"==a.type)if(a.min_rows?+a.min_rows!=a.min_rows?c.tabs[b].fields.min_rows.error="Please a valid number for mininum rows.":1>a.min_rows||10<a.min_rows?c.tabs[b].fields.min_rows.error=
...
"mandatory"==a.readonly_or_mandatory)"hidden"==a.visibility?c.tabs[b].fields.readonly_or_mandatory.error="A field cannot be mandatory while hidden.":"visible_on_condition"==a.visibility&&(c.tabs[b].fields.readonly_or_mandatory.error="This field is always mandatory but is conditionally visible. You must change this field to be either visible all the time or conditionally mandatory with the same condition as it's visibility.");else if("conditional_mandatory"==a.readonly_or_mandatory)if(a.mandatory_condition_field_id?
a.mandatory_condition_field_value||(f=this.getItem("field",a.mandatory_condition_field_id))&&-1!=["checkbox","group"].indexOf(f.type)&&(c.tabs[b].fields.mandatory_condition_field_value.error="Please select a mandatory on condition form field value."):c.tabs[b].fields.mandatory_condition_field_id.error="Please select a mandatory on condition form field.","hidden"==a.visibility)c.tabs[b].fields.visibility.error="A field cannot be mandatory while hidden.";else if("visible_if_one_of"==a.visible_condition_field_type&&
...
a.mandatory_condition_field_type)if(error=0,_._isEm(a.mandatory_condition_checkboxes_field_value)&&""!=a.mandatory_condition_field_id)c.tabs[b].fields.mandatory_condition_field_value.error="Please select a checkbox for mandatory condition.";else{if("visible"==a.visibility)error=0;else if(a.visible_condition_field_id!=a.mandatory_condition_field_id)error=1;else switch(a.visible_condition_field_type){case "visible_if_one_of":if(""!=a.visible_condition_checkboxes_field_value&&a.mandatory_condition_checkboxes_field_value)for(f=
0;f<a.mandatory_condition_checkboxes_field_value.length;f++)if(!_.contains(a.visible_condition_checkboxes_field_value,a.mandatory_condition_checkboxes_field_value[f])){error=1;break}break;case "visible_if_not":if(""==a.visible_condition_field_value&&!_._isEm(a.mandatory_condition_checkboxes_field_value)||""==a.visible_condition_field_value&&_._isEm(a.mandatory_condition_checkboxes_field_value)||_.contains(a.mandatory_condition_checkboxes_field_value,a.visible_condition_field_value))error=1;break;
case "visible_if":if(""!=a.visible_condition_field_value)if(a.mandatory_condition_checkboxes_field_value&&1!=a.mandatory_condition_checkboxes_field_value.length)error=1;else if(a.mandatory_condition_checkboxes_field_value)for(f=0;f<a.mandatory_condition_checkboxes_field_value.length;f++)if(a.mandatory_condition_checkboxes_field_value[f]!=a.visible_condition_field_value){error=1;break}}0!=error&&(c.tabs[b].fields.mandatory_condition_field_value.error="A field cannot be mandatory while hidden. If this field is both mandatory and visible on a condition, both fields and values must be the same.")}else"visible_on_condition"==
...
"Another field already has this code name on this form.");a.invalid_responses&&a.invalid_responses.length&&!a.invalid_field_value_error_message&&(c.tabs[b].fields.invalid_field_value_error_message.error="Please enter an error message when an invalid response is chosen.");if("textarea"==a.type&&(a.word_count_max&&(+a.word_count_max!=a.word_count_max?c.tabs[b].fields.word_count_max.error="Please enter a valid number for the max word count.":1>a.word_count_max&&(c.tabs[b].fields.word_count_max.error=
"Word count must be greater than 0.")),a.word_count_min&&(+a.word_count_min!=a.word_count_min?c.tabs[b].fields.word_count_min.error="Please enter a valid number for the min word count.":1>a.word_count_min&&(c.tabs[b].fields.word_count_min.error="Word count must be greater than 0.")),a.rows))if(+a.rows!=a.rows)c.tabs[b].fields.rows.error="Please enter a valid number for the rows.";else if(1>a.rows||100<a.rows)c.tabs[b].fields.rows.error="Rows must be greater than 0 and less than 100.";break;case "crm":a.send_to_crm&&
!a.field_crm_name&&(c.tabs[b].fields.field_crm_name.error="Please enter a CRM field name.")}};h.getTUIXFieldCustomValues=function(d){var a={};if("centralised_lists"==d)a=JSON.parse(JSON.stringify(this.tuix.centralised_lists.values));else if("centralised_list_filter_fields"==d||"conditional_fields"==d||"mirror_fields"==d)for(var b=this.getOrderedPages(),c=1<b.length,f=0,e=0;e<b.length;e++)for(var g=b[e],k=this.getOrderedFields(b[e].id),h=0;h<k.length;h++){var n=k[h];if(this.canAddFieldToList(d,n.id)&&
(a[n.id]={label:n.name,ord:++f},c)){var p="page_"+g.id;a[n.id].parent=p;a[p]||(a[p]={label:g.name,ord:++f})}}else"field_lov"==d&&"field"==this.editingThing&&this.editingThingId&&(n=this.getItem(this.editingThing,this.editingThingId))&&n.lov&&(a=JSON.parse(JSON.stringify(n.lov)));return a};h.canAddFieldToList=function(d,a){var b=this.tuix.items[a];return"centralised_list_filter_fields"==d?("field"!=this.editingThing||b.id!=this.editingThingId)&&("centralised_select"==b.type||"text"==b.type&&b.enable_suggested_values&&
"pre_defined"==b.suggested_values&&b.suggested_values_source):"conditional_fields"==d?("field"!=this.editingThing||b.id!=this.editingThingId)&&-1!="checkbox group radios select centralised_radios centralised_select checkboxes".split(" ").indexOf(b.type):"mirror_fields"==d?("field"!=this.editingThing||b.id!=this.editingThingId)&&-1!=["text","calculated","select","centralised_select"].indexOf(b.type):!1};h.saveCurrentOpenDetails=function(d){var a=[],b=this.editingThing,c=this.editingThingId;if(b&&c&&
this.editingThingTUIXTabId){var f=this.getTUIXModeForItemType(b),e=this.tuix[f],g=this.getItem(b,c);if(g){this.saveTUIXTab(e,this.editingThingTUIXTabId,g);if(d)a=this.getDeleteErrors(b,c);else{g.name=$("#field__name").val().trim();d=this.getTUIXTags(f,g,b);this.validateTUIX(b,g,this.editingThingTUIXTabId,d);for(var k in d.tabs)if(q(d.tabs,k)){var g=d.tabs[k],h;for(h in g.fields)q(g.fields,h)&&(tuixField=g.fields[h],tuixField.error&&a.push(tuixField.error))}}a.length&&this.openItemTUIXTab(b,c,this.editingThingTUIXTabId,
a)}}return 0==a.length};h.getDeleteErrors=function(d,a){var b=[];if("field"==d){this.getItem(d,a);for(var c=this.getOrderedFields(),f=0;f<c.length;f++){var e=c[f],g=!1;if("calculated"==e.type&&e.calculation_code)for(var k=0;k<e.calculation_code.length;k++){var h=e.calculation_code[k];if("field"==h.type&&h.value==a){g=!0;break}}if(e.visibility&&"visible_on_condition"==e.visibility&&e.visible_condition_field_id==a||e.readonly_or_mandatory&&"conditional_mandatory"==e.readonly_or_mandatory&&e.mandatory_condition_field_id==
a)g=!0;g&&b.push('Unable to delete field because the field "'+e.name+'" depends on it.')}}return b};h.displayPageFieldOrderErrors=function(){if(!this.getItem("page",this.currentPageId))return!0;for(var d=[],a=this.getOrderedFields(this.currentPageId),b="checkbox checkboxes date radios centralised_radios select centralised_select text textarea url attachment section_description calculated group".split(" "),c=!1,f=0;f<a.length;f++)field=a[f],"repeat_start"!=field.type||c?"repeat_end"==field.type?c?
c=!1:d.push("Repeat ends must be placed after repeat starts."):c&&-1==b.indexOf(field.type)&&d.push('Field type "'+this.getFieldReadableType(field).toLowerCase()+'" is not allowed in a repeat block.'):c=!0;a=0<d.length;b=$("#organizer_fields_error");b.html("");for(f=0;f<d.length;f++)b.append('<p class="error">'+d[f]+"</p>");b.toggle(a);return!a};h.getTUIXModeForItemType=function(d){return"page"==d?"form_page_details":"field"==d?"form_field_details":!1};h.getFieldReadableType=function(d){if("string"==
typeof d)var a=d;else a=d.type,d.is_consent&&(a="consent");switch(a){case "checkbox":return"Checkbox";case "checkboxes":return"Checkboxes";case "date":return"Date";case "editor":return"Editor";case "radios":return"Radios";case "centralised_radios":return"Centralised radios";case "select":return"Select";case "centralised_select":return"Centralised select";case "text":return"Text";case "textarea":return"Textarea";case "url":return"URL";case "attachment":return"Attachment";case "page_break":return"Page";
case "section_description":return"Subheading";case "calculated":return"Calculated";case "restatement":return"Mirror";case "repeat_start":return"Start of repeating section";case "repeat_end":return"End of repeating section";case "document_upload":return"Multi-upload";default:return"Unknown";case "group":return"Group";case "consent":return"Consent";case "file_picker":return"File picker"}};h.getCalculationCodeDisplay=function(d){var a="";if(d)for(var b=!1,c=0;c<d.length;c++){b||"parentheses_close"==
d[c].type?b&&(b=!1):a+=" ";switch(d[c].type){case "operation_addition":a+="+";break;case "operation_subtraction":a+="-";break;case "operation_multiplication":a+="\u00d7";break;case "operation_division":a+="\u00f7";break;case "parentheses_open":a+="(";b=!0;break;case "parentheses_close":a+=")";break;case "static_value":a+=d[c].value;break;case "field":var f="",f=(f=this.getItem("field",d[c].value))?f.name:"UNKNOWN FIELD",a=a+('"'+f+'"')}a=a.trim()}return a};h.calculationAdminBoxAddSomthing=function(d,
a){var b=!1;switch(d){case "operation_addition":case "operation_subtraction":case "operation_multiplication":case "operation_division":case "parentheses_open":case "parentheses_close":b={type:d};break;case "static_value":""===a||isNaN(+a)||(b={type:d,value:+a},$("#static_value").val(""));break;case "field":b={type:d,value:a},$("#numeric_field").val("")}if(b){var c=[];$("#calculation_code").val()&&(c=JSON.parse($("#calculation_code").val()));c.push(b);$("#calculation_code").val(JSON.stringify(c));
this.calculationAdminBoxUpdateDisplay(c)}};h.calculationAdminBoxDelete=function(){var d=[];$("#calculation_code").val()&&(d=JSON.parse($("#calculation_code").val()))&&(d.pop(),$("#calculation_code").val(JSON.stringify(d)));this.calculationAdminBoxUpdateDisplay(d)};h.calculationAdminBoxUpdateDisplay=function(d){d=this.getCalculationCodeDisplay(d);$("#zenario_calculation_display").text(d)};h.openFormSettings=function(){var d=this;x.open("zenario_user_form",{id:v.pi.refiner.id},r,r,function(a,b){var c=
"";b.details.show_title&&(c=b.details.title);d.tuix.title=c;$("#organizer_form_builder .form_outer .form_header h5").text(c)})};h.changeMadeToPanel=function(){this.changeMadeOnPanel||(this.changeMadeOnPanel=!0,y.onbeforeunload=function(){return"You are currently editing this form. If you leave now you will lose any unsaved changes."},v._dI("Please either save your changes, or click Reset to discard them, before exiting the form editor."),v._sB())};h.saveChanges=function(){var d=this;if(d.tuix.not_used_or_on_public_page){for(var a=
!1,b=!1,c=d.getOrderedFields(),f=0;f<c.length;f++)c[f].dataset_field_id&&("email"==c[f].db_column?b=!0:a=!0);if(a&&!b&&!confirm("Warning: this form contains fields that are linked to the Users Dataset, but doesn't contain the Email field from the Users Dataset.\n\nIf this form is used on a public web page by anonymous visitors, form responses will not be stored in the Users & Contacts database table.\n\nTo overcome this, please add the Email field linked to the Users Dataset.\n\nSave anyway?"))return}a=
