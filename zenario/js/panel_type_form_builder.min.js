...
f={};for(d in h)m(h,d)&&(c=h[d],f[d]={ord:c.ord,label:c.name});a.restatement_field_options=this.createSelectList(f,a.restatement_field);a.default_value_mode_options=this.createRadioList({none:{ord:1,label:"No default value"},value:{ord:2,label:"Enter a default value"},method:{ord:3,label:"Call a modules static method to get the default value"}},a.default_value_mode,"default_value_mode");d={};"checkbox"==a.type||"group"==a.type?d={0:{ord:1,label:0},1:{ord:2,label:1}}:-1!=["radios","centralised_radios",
...
("select"==a.type||"radios"==a.type)));a.showDetailsTab=!0;a.showAdvancedTab="page_break"!=a.type&&"section_description"!=a.type;a.showValuesTab=-1!=["select","radios","checkboxes"].indexOf(a.type);a.showTranslationsTab=this.tuix.show_translation_tab&&"page_break"!=a.type;a.showCRMTab=a.crm_enabled&&"page_break"!=a.type&&"section_description"!=a.type;return a};g.initCurrentFieldDetails=function(){var b=this,a=this.tuix.items[this.current.id]||{};$("#organizer_field_details :input").off().on("change",
function(){b.changeMadeToPanel()});$('#organizer_field_details input[type="text"], #organizer_field_details textarea, #zenario_field_details_header_content input[type="text"]').on("keyup",function(){b.changeMadeToPanel()});$("#field__field_label").on("keyup",function(){var c=$(this).val();$("#organizer_form_field_"+b.current.id+" .label").text(c);a.just_added&&$("#field__name").val(c)});a.field_id||($("#zenario_field_details_header_content .edit_field_name_button").off().on("click",function(){$("#zenario_field_details_header_content .view_mode").hide();
