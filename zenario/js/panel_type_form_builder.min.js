...
g=!0);h.phrases=[];var l,k;for(l in f)m(f,l)&&(k=f[l],parseInt(k.translate_phrases)&&h.phrases.push({field_column:h.column,language_id:k.id,language_name:k.english_name,phrase:d.phrases[k.id]&&!g?d.phrases[k.id]:"",disabled:g}));c.push(h)}c.sort(this.sortByOrd);return c};g.getOrderedDatasetFields=function(b){var a,d,c,e,f,h,g=[],l={};for(h in this.tuix.items)m(this.tuix.items,h)&&(f=this.tuix.items[h],f.dataset_field_id&&(l[f.dataset_field_id]=!0));for(d in b)if(m(b,d)){c=b[d];a={};for(e in c)m(c,
e)&&(f=c[e],"fields"!=e&&(a[e]=f));if(c.fields){a.fields=[];for(h in c.fields)m(c.fields,h)&&(f=c.fields[h],!0!==l[h]&&a.fields.push(f));a.fields.sort(this.sortByOrd)}g.push(a)}g.sort(this.sortByOrd);return g};g.showPanel=function(b,a,d){b.html("").hide();this.changesSaved&&(this.changesSaved=!1,p._t({message_type:"success",message:"Your changes have been saved!"}));var c=this;b=_._cl(this.tuix.items);var e={items:{},field:{},details_section:"field_type_list",form_title:this.tuix.form.title};this.currentFieldTab=
this.currentFieldTab?this.currentFieldTab:"details";"translations"!=this.currentFieldTab||this.tuix.show_translation_tab||(this.currentFieldTab="details");this.current&&this.current.id&&this.tuix.items[this.current.id]?(e.items=this.getOrderedItems(b),!this.tuix.items[this.current.id]&&this.currentFieldOrd!==n&&e.items[this.currentFieldOrd]&&(this.current.id=e.items[this.currentFieldOrd].id),e.details_section="field_details",e.field=this.getCurrentFieldDetailsMergeFields(),b[this.current.id].selected=
!0):this.current={id:!1,type:!1};e.items=this.getOrderedItems(b);b=this.microTemplate(k.z+"organizer_form_builder",e);a.html(b).show();this.current&&this.initCurrentFieldDetails();this.maxNewCustomFieldValue=this.maxNewCustomField=this.maxNewCustomTab=1;this.deleting=!1;this.pageBreakCount=this.tuix.pageBreakCount;this.initSection();$("#organizer_field_type_list div.field_type, #organizer_centralised_field_type_list div.field_type").draggable({connectToSortable:"#organizer_form_fields",appendTo:"#organizer_form_builder",
...
b=this.microTemplate(k.z+"organizer_form_builder_dataset_tab",b);$("#organizer_linked_field_type_list").html(b);$("#organizer_linked_field_type_list div.dataset_field").draggable({connectToSortable:"#organizer_form_fields",appendTo:"#organizer_form_builder",helper:"clone"})};g.initDeleteButtons=function(){var b=this;$("#organizer_form_fields .delete_icon").off().on("click",function(a){var d=$(this).data("id");if("registration"==b.tuix.form.type&&"email"==b.tuix.items[d].db_column)return p._sM("You cannot delete the email field from a registration form.",
!0,"warning"),!1;a={};var c=b.tuix.items[d].type,e,f;for(e in b.tuix.items)m(b.tuix.items,e)&&(f=b.tuix.items[e],f.type==c&&e!=d&&(a[e]=f.name));e={id:d,field_name:b.tuix.items[d].name,field_type:c,field_english_type:b.getFieldReadableType(c),responses_transfer_fields:JSON.stringify(a)};b.noTransition=!0;q.open(k.z+"delete_form_field",e,n,n,function(a,c){"delete_field_but_migrate_data"==c.details.delete_field_options&&c.details.migration_field&&b.tuix.items[c.details.migration_field]&&(b.tuix.items[c.details.migration_field]._migrate_responses_from=
d);var f=!0;setTimeout(function(){!0==f&&b.deleteField(d);f=!1})})})};g.deleteField=function(b){var a=this,d=function(c){if(_._isEm(c)){var d,f;b===n&&(b=a.current.id);c=a.tuix.items[b];if("page_break"==c.type)a.pageBreakCount--;else if("repeat_start"==c.type)for(var h=!1,g=a.getOrderedItems(a.tuix.items),l=0;l<g.length;l++)if(g[l].id==b)h=!0;else if(h&&"repeat_end"==g[l].type){a.deleteField(g[l].id);break}a.tuix.items[b]={remove:!0};d=$("#organizer_form_field_"+b);c.dataset_field_id&&a.initLinkedFieldsAdder();
d&&(f=a.selectNextField(d),a.changeMadeToPanel(),d.animate({height:0,opacity:0},500,function(){d.remove();!1===f&&a.showNoFieldsMessage()}));a.updateFieldsContainerHeight()}};(cb=this.ajaxRequestOnChange(!0))?cb.after(d):d([])};g.selectNextField=function(b){for($next=b.prev();1==$next.length;)if(this.tuix.items[$next.data("id")]&&this.tuix.items[$next.data("id")].remove)$next=$next.prev();else break;if(0==$next.length)for($next=b.next();1==$next.length;)if(this.tuix.items[$next.data("id")]&&this.tuix.items[$next.data("id")].remove)$next=
...
$("#organizer_form_field_inline_buttons_"+d.current.id).show(),d.showDetailsSection("organizer_field_details",!!a))},e=b.data("id");"repeat_end"!=this.tuix.items[e].type&&(this.save(),(cb=this.ajaxRequestOnChange())?cb.after(c):c([]))};g.setCurrentFieldDetails=function(){var b=this.getCurrentFieldDetailsMergeFields(),b=this.microTemplate(k.z+"organizer_form_builder_field_details",b);$("#organizer_field_details_inner").html(b);this.initCurrentFieldDetails()};g.getCurrentFieldDetailsMergeFields=function(){var b=
...
c.type||"text"==c.type&&1==c.autocomplete&&!1!=c.values_source)&&d!=this.current.id&&(f[d]=_._cl(c)),-1!="checkbox group radios select centralised_radios centralised_select".split(" ").indexOf(c.type)&&d!=this.current.id&&(h[d]={label:c.name,ord:c.ord}));"calculated"==b.type&&(c="",c=Array.isArray(b.calculation_code)?b.calculation_code:$.map(b.calculation_code,function(a){return a}),a.calculation_code_display=this.getCalculationCodeDisplay(c));c={};var g=1,l,k;for(l in this.tuix.centralised_lists.values)m(this.tuix.centralised_lists.values,
l)&&(k=this.tuix.centralised_lists.values[l],c[l]={ord:g++,label:k.label});a.values_source_options=this.createSelectList(c,a.values_source);this.tuix.centralised_lists.values[a.values_source]&&this.tuix.centralised_lists.values[a.values_source].info.can_filter&&(a.show_source_filter=!0,a.source_filter_label=this.tuix.centralised_lists.values[a.values_source].info.filter_label);l={};for(d in f)m(f,d)&&(c=f[d],l[d]={ord:c.ord,label:c.name});a.filter_on_field_options=this.createSelectList(l,a.filter_on_field,
!0);a.readonly_or_mandatory_options=this.createSelectList({none:{ord:1,label:"None"},mandatory:{ord:2,label:"Mandatory"},readonly:{ord:3,label:"Read-only"},conditional_mandatory:{ord:4,label:"Mandatory on condition"}},a.readonly_or_mandatory);a.mandatory_condition_field_id_options=this.createSelectList(h,a.mandatory_condition_field_id,!0);f=this.tuix.items[a.mandatory_condition_field_id];l={};c=!0;f&&("checkbox"==f.type||"group"==f.type?l={0:{label:"Unchecked",ord:1},1:{label:"Checked",ord:2}}:(l=
f.lov,c="-- Any value --"));""!==a.mandatory_condition_field_id&&a.mandatory_condition_field_id!==n&&(a.mandatory_condition_field_value_options=this.createSelectList(l,a.mandatory_condition_field_value,c));f={visible:{ord:1,label:"Visible"},hidden:{ord:2,label:"Hidden"},visible_on_condition:{ord:3,label:"Visible on condition"}};"page_break"==b.type&&delete f.hidden;a.visibility_options=this.createSelectList(f,a.visibility);a.visible_condition_field_id_options=this.createSelectList(h,a.visible_condition_field_id,
!0);f=this.tuix.items[a.visible_condition_field_id];l={};c=!0;f&&("checkbox"==f.type||"group"==f.type?l={0:{label:"Unchecked",ord:1},1:{label:"Checked",ord:2}}:(l=f.lov,c="-- Any value --"));""!==a.visible_condition_field_id&&a.visible_condition_field_id!==n&&(a.visible_condition_field_value_options=this.createSelectList(l,a.visible_condition_field_value,c));a.validation_options=this.createSelectList({none:{ord:1,label:"None"},email:{ord:2,label:"Email"},URL:{ord:3,label:"URL"},number:{ord:4,label:"Number"},
...
$("#field__css_classes").on("keyup",function(){var a=$(this).val();$("#organizer_form_field_"+b.current.id+" .form_field_classes .css").toggle(!!a)});$("#field__div_wrap_class").on("keyup",function(){var a=$(this).val();$("#organizer_form_field_"+b.current.id+" .form_field_classes .div").toggle(!!a)});$(k.h+"field_details_header_content .edit_field_name_button").off().on("click",function(){$(k.h+"field_details_header_content .view_mode").hide();$(k.h+"field_details_header_content .edit_mode").show()});
$(k.h+"field_details_header_content .done_field_name_button").off().on("click",function(){$(k.h+"field_details_header_content .edit_mode").hide();$(k.h+"field_details_header_content .view_mode").show();var a=$(k.h+'field_details_header_content .edit_mode input[name="name"]').val();$(k.h+"field_details_header_content .view_mode h5").text(a)});$("#field__note_to_user").on("keyup",function(){var a=$(this).val(),c=$("#organizer_form_field_note_below_"+b.current.id);$("#organizer_form_field_note_below_"+
...
default:return"Unknown"}};g.centralisedListUpdated=function(b,a,d){var c=this;this.sendAJAXRequest({mode:"get_centralised_lov",method:a,filter:d,type:"object"}).after(function(a){a=JSON.parse(a);c.tuix.items[b].lov=a;c.valuesChanged=!0;a=c.getOrderedItemCRMLOV(a);a=c.microTemplate(k.z+"organizer_admin_box_builder_radio_values_preview",a);$("#organizer_form_field_values_"+b).html(a)})};g.getCurrentFieldDetails=function(){var b={},a=this.tuix.items[this.current.id];$.each($("#organizer_field_details_form :input:not([disabled], [type=button], .crm_value_input, .translation)").serializeArray(),
function(a,c){c.name&&(b[c.name]=c.value)});$("#organizer_field_details_form input[type=checkbox]").map(function(a,c){b[c.name]=c.checked?1:0});b.default_value_lov!==n?b.default_value=b.default_value_lov:b.default_value_text!==n&&(b.default_value=b.default_value_text);-1==["select","radios","checkboxes"].indexOf(a.type)||a.dataset_field_id||(b.lov=this.getCurrentFieldValues(a));this.tuix.show_translation_tab&&$("#field_section__translations input.translation").map(function(b,c){var e=$(this).data("language_id"),
...
return b};g.save=function(){var b={},a,d;if(this.current.id){b=this.getCurrentFieldDetails();if(!this.tuix.items[this.current.id])this.tuix.items[this.current.id]={};else if(this.tuix.items[this.current.id].remove)return;for(a in b)m(b,a)&&(d=b[a],"field_crm_name"==a||"send_to_crm"==a?this.tuix.items[this.current.id]._crm_data[a]=d:this.tuix.items[this.current.id][a]=d)}};g.ajaxRequestOnChange=function(b){var a=this,d,c,e=new u.callback,f={mode:"validate_field",id:this.current.id,field_tab:this.currentFieldTab,
...
'" is not allowed in repeat blocks.':"invalid_repeat_block_end"==a.error.code&&(c="You cannot have a Repeat End before a Repeat Start."),$("#organizer_fields_error").show().text(c),c=!0,b.changesSaved=!1);p._nDS();c||(b.changingForm=!1,r._eI(),t.onbeforeunload=!1,r._r());b.saveLock=!1})};g.initFormSettingsButton=function(){$("input.form_settings").off().on("click",function(){q.open(k.z+"user_admin_box_form",{id:r.pi.refiner.id},n,n,function(b,a){var d="";a.details.show_title&&(d=a.details.title);
...
b)this.fieldClick($("#organizer_form_field_"+this.current.id),"translations");else{if(!a&&(this.save(),c=this.ajaxRequestOnChange())){c.after(e);return}e([])}}},zenarioO.panelTypes);
