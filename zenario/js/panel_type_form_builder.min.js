...
d.just_added&&$("#field__name").val($(this).val())}),$("#field__note_to_user").on("keyup",function(){var b=$(this).val().trim();$("#organizer_form_field_note_below_"+a+" div.zenario_note_content").text(b);$("#organizer_form_field_note_below_"+a).toggle(""!==b)}),"text"==d.type||"textarea"==d.type)$("#field__placeholder").on("keyup",function(){$("#organizer_form_field_"+a+" :input").prop("placeholder",$(this).val())});else if("section_description"==d.type)$("#field__description").on("keyup",function(){$("#organizer_form_field_"+
a+" .description").text($(this).val())});else{if("calculated"==d.type)$("#edit_field_calculation").on("click",function(){var b={},c;for(c in k.tuix.fields)if(m(k.tuix.fields,c)){var e=k.tuix.fields[c];if("text"==e.type&&-1!=["number","integer","floating_point"].indexOf(e.field_validation)||"calculated"==e.type&&a!=c)b[c]={label:e.name,ord:e.ord}}c={id:a,title:'Editing the calculation for the field "'+d.name+'"',calculation_code:JSON.stringify(d.calculation_code)};b={details:{dummy_field:JSON.stringify(b)}};
x.open(t+"field_calculation",c,p,b,function(b,c){d.calculation_code=c.details.calculation_code?JSON.parse(c.details.calculation_code):"";k.changeMadeToPanel();k.saveCurrentOpenDetails();k.loadFieldDetailsPage("details",a)})})}else if("advanced"==b)$("#field__css_classes").on("keyup",function(){$("#organizer_form_field_"+a+" .form_field_classes .css").toggle(!!$(this).val()).prop("title",$(this).val())}),$("#field__div_wrap_class").on("keyup",function(){$("#organizer_form_field_"+a+" .form_field_classes .div").toggle(!!$(this).val()).prop("title",
$(this).val())});else if("values"==b)q.values&&!q.values._hidden&&(f=this.getOrderedFieldValues(a),e=this.microTemplate(t+"organizer_admin_box_builder_field_value",f),$("#field_values_list").html(e).sortable({containment:"parent",tolerance:"pointer",axis:"y",start:function(a,c){k.startIndex=c.item.index()},stop:function(c,b){k.startIndex!=b.item.index()&&(k.saveFieldListOfValues(k.tuix.fields[a]),k.loadFieldValuesListPreview(a),k.changeMadeToPanel())}}),$("#field_values_list input").on("keyup",function(){var a=
