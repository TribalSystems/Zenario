...
1*b.val());(b=$("#"+h+" input.zenario_grid_setting_gutter_flu")).length&&(a.gutterFlu=1*b.val());(b=$("#"+h+" input.zenario_grid_setting_gutter_right_flu")).length&&(a.gutterRightEdgeFlu=1*b.val())};a.updateAndChange=function(){a.update();a.change()};a.update=function(){a.readSettings(a.data);a.checkData()};a.recalcOnKeyUp=function(){a.recalc(this)};a.recalcOnChange=function(){a.recalc(this,!0,!0)};a.recalcOnInc=function(c,b){if(c.originalEvent&&"mouseup"==c.originalEvent.type)return a.recalc(this,
...
a.add=function(c,b,d){d=$(c).attr("data-levels");c=$(c).attr("data-new-width")||1;var g=a.data;$.colorbox.close();if(y(d)&&!1!==d){if(d){d=d.split("-");for(var h in d)s(d,h)&&(h*=1,g=g.cells[1*d[h]])}"slot"==b?(g.cells.push({width:c,slot:!0,name:a.uniqueRandomName(),flash:!0}),toastr.success(m.growlSlotAdded)):"space"==b?(g.cells.push({width:c,space:!0,css_class:a.randomName(2,"Space_"),flash:!0}),toastr.success(m.growlSpaceAdded)):"grouping"==b?(g.cells.push({width:c,css_class:a.randomName(2,"Grouping_"),
...
function(c,b){var d;if((d=$(c).attr("data-for"))&&(d=$("#"+d))){var g=d.attr("data-i");d=d.attr("data-levels");var h=a.data;if(d){d=d.split("-");for(var k in d)s(d,k)&&(k*=1,h=h.cells[1*d[k]])}if(h.cells[g].slot){if(b.name){if(b.name!=b.name.replace(/[^a-zA-Z0-9_]/g,"")){$("#zenario_grid_error").html(m.gridErrorNameFormat).slideDown();return}if(b.name!=h.cells[g].name&&a.checkIfNameUsed(b.name)){$("#zenario_grid_error").html(m.gridErrorNameInUse).slideDown();return}}else{$("#zenario_grid_error").html(m.gridErrorNameIncomplete).slideDown();
