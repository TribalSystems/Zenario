...
onComplete:function(){$("#zenario_grid_slotname_button").click(function(){var b,e={},f=$("#colorbox .zenario_grid_properties_form").serializeArray();for(b in f)u(f,b)&&(e[f[b].name]=f[b].value);a.saveProperties(c,e)});$(".zenario_grid_properties_form input,textarea")[1].focus()}})};a.ajaxURL=function(){return v+"zenario/admin/grid_maker/ajax.php"};a.ajaxSrc=function(){var c=a.ajaxData(),b=a.ajaxURL()+"?data="+x(c);400<b.length&&(b=a.ajaxURL()+"?cdata="+h._nAA(a.ajaxURL(),{compress:1,data:c}));return b};
a.drawPreview=function(){a.clearAddToolbar();a.checkData();var c=Math.max(a.desktopSmallestSize,100*Math.round(($(t).width()-a.previewPaddingLeftRight)/100)),b=Math.min(c-20,a.data.maxWidth),e=a.microTemplate(a.mtPrefix+"preview",{gridId:a.gridId,topHack:5,leftHack:10,min:100,max:c,startingValue:b});$("#"+a.gridId).css("margin","auto");$(".ui-tooltip").remove();l(a.gridId).innerHTML=e;n._to("#"+a.gridId+"  *[title]");$('<form action="'+r(a.ajaxURL())+'" method="post" target="'+a.gridId+'-iframe"><input name="data" value="'+
r(a.ajaxData())+'"/></form>').appendTo("body").hide().submit().remove();$("#"+a.gridId+"-slider").slider({min:100,max:c,step:10,animate:!0,value:b,start:function(b,c){$("#"+a.gridId+"-slider_overlay").css("display","block")},stop:function(b,c){$("#"+a.gridId+"-slider_overlay").css("display","none")},slide:function(b,c){$("#"+a.gridId+"-slider_val").val(c.value);$("#"+a.gridId+"-iframe").clearQueue();$("#"+a.gridId+"-size").clearQueue();!b.originalEvent||"mousedown"!=b.originalEvent.type&&"touchstart"!=
...
"add_toolbar",{}));$("#"+a.addToolbarId+" a").draggable({connectToSortable:".zenario_grids",helper:"clone",revert:"invalid"});var c=$("#"+a.addToolbarId),b=c.position();b&&q(b.top)&&q(b.left)&&c.css({right:"auto",top:b.top,left:b.left});$("#"+a.addToolbarId).draggable({cursor:"move"})};$(w).ready(function(){$("#"+a.addToolbarId).hover(function(){$(this).css("cursor","move")},function(){$(this).css("cursor","pointer")})});a.resizePreview=function(){var c,b=0;(c=l(a.gridId+"-iframe"))&&c.contentWindow&&
