...
g.zenario&&g.zenario.cID?g.location.reload(!0):g.$.colorbox.close()}):l("close_button").style.display="none";h.startPoking(a)};a.draw=function(c){c||a.drawForm();a.editing?a.drawEditor():a.drawPreview()};a.drawForm=function(){var c=a.microTemplate(a.mtPrefix+"top",{formId:"settings"});$(".ui-tooltip").remove();l("settings").innerHTML=c;n._to("#settings");a.data.fluid||a.recalcColumnAndGutterOptions(a.data);$("#settings input.zenario_grid_setting_break_1").change(a.update).spinner({stop:a.update,min:10,
