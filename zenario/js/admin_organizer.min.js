...
s(a.tuix.trash.empty)?u(a.tuix.trash.empty_css_class,"trash_button_empty"):u(a.tuix.trash.full_css_class,"trash_button_full"),n.tooltips("#organizer_trash_button a",{position:{my:"center bottom-12",at:"center top",collision:"flipfit",using:k.tooltipsUsing},items:"*",content:s(a.tuix.trash.empty)?u(a.tuix.trash.empty_tooltip,q.viewTrash):u(a.tuix.trash.full_tooltip,q.viewTrash)})):m("organizer_trash_button").style.display="none"};a.getPanelItems=function(b){var c=!1,d,e,f=-1,g=-1,h=!1,m="firstcell ",
l={},n=0,u={},t=_.clone(a.pi.returnSelectedItems()),p={items:[],columns:[],totalWidth:0,maxNumberOfInlineButtons:0,allItemsSelected:a.allItemsSelected()};for(e in a.sortedColumns)k.has(a.sortedColumns,e)&&(d=a.sortedColumns[e],a.isShowableColumn(d,!0)&&(h=d));for(e in a.sortedColumns)if(k.has(a.sortedColumns,e)){d=a.sortedColumns[e];if(a.boldColsInListView==d||-1!==a.boldColsInListView.indexOf("[["+d+"]]"))u[d]=!0;a.isShowableColumn(d,!0)&&(l[d]=a.prefs[a.voPath]&&a.prefs[a.voPath].colSizes&&a.prefs[a.voPath].colSizes[d]?
Math.max(a.prefs[a.voPath].colSizes[d],a.columnWidths.xxsmall):a.tuix.columns[d].width,l[d]&&a.columnWidths[l[d]]?l[d]=a.columnWidths[l[d]]:(l[d]*=1)||(l[d]=a.defaultColumnWidth),p.totalWidth&&(p.totalWidth+=a.columnSpacing),p.totalWidth+=l[d]+a.columnPadding,p.columns[++f]={id:d,tuix:a.tuix.columns[d],htmlId:"organizer_column__"+d,css_class:m+(h==d?"lastcell":"")+(u[d]?" label_column":"")+a.columnCssClass(d),title:a.tuix.columns[d].title,tooltip:a.tuix.columns[d].tooltip,width:l[d]},m="")}p.totalWidth+=
...
" organiser_item_with_image",p.items[g].image=k.addBasePath(a.tuix.items[z].image));p.items[g].tooltip=a.tuix.items[z].tooltip;!p.items[g].tooltip&&a.tuix.item&&(a.tuix.item.tooltip_when_link_is_active&&a.itemClickThroughLink(z)?p.items[g].tooltip=a.applyMergeFields(a.tuix.item.tooltip_when_link_is_active,!0,z):a.tuix.item.tooltip&&(p.items[g].tooltip=a.applyMergeFields(a.tuix.item.tooltip,!0,z)));var J=-1,m="firstcell ",K=!1,P=!1,E;for(e in a.sortedColumns)k.has(a.sortedColumns,e)&&(d=a.sortedColumns[e],
a.isShowableColumn(d,!0)&&(E=a.columnValue(z,d),K=!(""==E&&0!==E),p.items[g].cells[++J]={id:d,first:!!m,css_class:"organizer_column__"+d+"__cell "+f+m+(h==d?"lastcell":"")+(s(a.tuix.columns[d].align_right)?" right":"")+(u[d]?" label_column":"")+a.columnCssClass(d,z),value:E,needsComma:K&&P,width:l[d]},m="",P=K&&!(E+"").match(/\W\s*$/)));p.items[g].inline_buttons=a.getInlineButtons(z);p.items[g].inline_buttons&&(n=p.items[g].inline_buttons.length)&&n>p.maxNumberOfInlineButtons&&(p.maxNumberOfInlineButtons=
...
0;a.updateYourWorkInProgress=function(){var b=(new Date).getTime();if(a.yourWorkInProgressLastUpdated-b<-a.yourWorkInProgressLastUpdateFrequency){a.yourWorkInProgressLastUpdated=b;var c=m("zenario_ywip_dropdown");c&&($(c).removeClass("zenario_ywip_loaded").addClass("zenario_ywip_loading"),setTimeout(function(){var b=k.nonAsyncAJAX(J+"zenario/admin/ajax.php?_json=1&path=zenario__content/panels/content&_sort_col=last_modified_datetime&_sort_desc=1&_start=0&_limit="+a.yourWorkInProgressItemCount+"&refinerId=&refinerName=your_work_in_progress&refiner__your_work_in_progress=&_get_item_data=1",
!1,!0,!0),b=n.microTemplate("zenario_ywip",b);c.innerHTML=b;$(c).removeClass("zenario_ywip_loading").addClass("zenario_ywip_loaded")},1))}};$(S).ready(a.updateYourWorkInProgress);a.sizing=!1;a.lastSize=!1;a.size=function(b){var c=46,d=66,e=$("#zenario_fbog");a.sizing&&clearTimeout(a.sizing);if(m("organizer__box_wrap")){var f,g;n.storekeeperWindow?(f=Math.floor($(h).width()),g=Math.floor($(h).height())):(f=Math.floor(e.width()),g=Math.floor(e.height()));if(f&&g&&(550>f&&(f=550),400>g&&(g=400),b||a.lastSize!=
f+"x"+g)){b=m("organizer_rightColumnContent");var l=m("organizer_lowerLeftColumn"),q=m("organizer_lowerMiddleColumn"),s=m("organizer_lowerRightColumn"),t=$("#organizer_header"),u=$(b),w=$(q),p=$("#organizer_buttons");e.length&&(m("zenario_fbog").className=n.getSKBodyClass());n.storekeeperWindow||e.height(g);a.tuix&&a.tuix.banner||(d=0);h.zenarioONotFull&&--c;m("organizer_leftColumn").style.height=1*g-46-c-2+"px";m("organizer_rightColumn").style.height=m("organizer_preloader_circle").style.height=
1*g-46-c+"px";m("organizer_Outer").style.height=1*g-46+"px";m("organizer_colsortfields")&&(k.browserIsIE(6)?m("organizer_colsortfields").style.height=1*g-46-130+"px":m("organizer_colsortfields").style.maxHeight=1*g-46-130+"px");c=1*g-46-c-40-d-30-2-a.columnPadding+0-55-13;b&&(b.style.height=c+"px");a.showLeftColumn?(c=204,m("organizer_leftColumn").style.display=m("organizer_leftColumnOuter").style.display=""):(c=0,m("organizer_leftColumn").style.display=m("organizer_leftColumnOuter").style.display=
"none");m("organizer__box_top").style.width=m("organizer__box_wrap").style.width=m("organizer__box_center").style.width=m("organizer__box_bottom").style.width=f-0+"px";m("organizer_Outer").style.width=f-22+"px";m("organizer_wrapInner_bm").style.width=f-32+"px";m("organizer_wrapOuter_tm").style.width=f-0+"px";m("organizer_rightColumn").style.width=m("organizer_preloader_circle").style.width=f-30-c+"px";m("organizer_rightColumnTitle")&&(m("organizer_rightColumnTitle").style.width=f-30-c-2+"px");b&&
(b.style.width=f-30-c-2+"px");h.zenarioOSelectMode?m("organizer_choose_button").style.display="inline":m("organizer_choose_button").style.display="none";825>=f?(l.style.width="40%",q.style.width="49%",s.style.width="10%"):965>=f?(l.style.width="35%",q.style.width="49%",s.style.width="15%"):1155>=f?(l.style.width="30%",q.style.width="49%",s.style.width="20%"):1155<=f&&(l.style.width="25%",q.style.width="49%",s.style.width="25%");a.setPanelTitle();a.pi&&a.pi.sizePanel(t,u,w,p);a.lastSize=f+"x"+g;a.setTopLevelNavScrollStatus()}k.browserIsIE()&&
m("organizer_rightColumnContent")&&(k.browserIsIE(8)?m("organizer_Outer").onselectstart=function(){return!1}:(m("organizer_leftColumnOuter").onselectstart=m("organizer_itemCountWrap").onselectstart=function(){return!1},m("organizer_rightColumn").onselectstart=function(){return!(1<a.itemsSelected)}));a.sizing=setTimeout(a.size,500)}}});
